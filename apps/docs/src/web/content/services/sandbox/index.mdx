...
Call `sandbox.resume()` when a sandbox was paused and you need it ready before issuing commands.

```typescript
await sandbox.resume();
const execution = await sandbox.execute({ command: ['bun', 'run', 'test'] });

ctx.logger.child({ executionId: execution.executionId }).info('Sandbox command completed');
```

<Callout type="info" title="Resume before batches">
Use explicit `sandbox.resume()` (or `agentuity cloud sandbox resume`) when you need the sandbox ready before issuing multiple commands.
</Callout>
