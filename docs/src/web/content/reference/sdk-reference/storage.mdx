---
title: Storage APIs (v2 Compatibility)
short_title: Storage
description: Compatibility reference for v2 runtime ctx.* storage APIs
---

This page documents the v2 runtime storage shape used by `ctx.*` examples. In v3 apps, start with [Storage](/services/storage) and [Using Standalone Packages](/reference/standalone-packages): use `KeyValueClient`, `VectorClient`, `StreamClient`, direct database clients, Bun S3 APIs, or `@agentuity/hono` when Hono routes should read clients from `c.var.*`.

<Callout type="info" title="Use direct clients for new v3 code">
The v3 storage clients expose the same core operations through `@agentuity/keyvalue`, `@agentuity/vector`, and `@agentuity/stream`. Keep the `ctx.kv`, `ctx.vector`, and `ctx.stream` examples here for existing runtime apps while you migrate.
</Callout>
...
The v2 runtime API uses `ctx.kv` in agents and `c.var.kv` in routes. The v3 `KeyValueClient` and Hono `c.var.kv` expose the same core methods.

<Callout type="warning" title="Local KV has a smaller API surface">
...
- `params` (optional):
  ...
  - `sort`: Sort field, one of `'name'`, `'size'`, `'records'`, `'created'`, or `'lastUsed'`
  - `direction`: `'asc'` or `'desc'`
  - `name`: Filter namespaces by name substring
  - `projectId`, `agentId`, `projectName`, `agentName`: Cloud metadata filters
...
The v2 runtime API uses `ctx.vector` in agents and `c.var.vector` in routes. The v3 `VectorClient` and Hono `c.var.vector` expose the same core methods.
...
The v2 runtime API uses `ctx.stream` in agents and `c.var.stream` in routes. The v3 `StreamClient` and Hono `c.var.stream` expose stream creation, download, listing, and delete methods.
...
- `params` (optional):
  - `namespace`: Filter by stream namespace. Takes precedence when `name` is also provided
  - `name`: Filter streams by namespace/name substring
  ...
  - `sort`: Sort field, one of `'name'`, `'created'`, `'updated'`, `'size'`, `'count'`, or `'lastUsed'`
  - `direction`: `'asc'` or `'desc'`
