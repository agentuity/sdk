...
If your cron route uses `{ auth: true }`, the request must include a valid cron signature header. In deployed environments, Agentuity adds these headers automatically. For local testing, temporarily set `{ auth: false }` or use the Agentuity CLI to trigger the route.
