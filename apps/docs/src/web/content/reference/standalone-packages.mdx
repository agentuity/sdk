...
| [`@agentuity/coder`](#coder) | `bun add @agentuity/coder` | Manage Coder sessions, participants, and replay data |
...
| [Coder](/services/coder) | N/A | N/A | `@agentuity/coder` | N/A |
...
<Callout type="info" title="Discovery before Hub requests">
Coder resolves your org's Hub URL dynamically. `CoderClient` calls the Agentuity API for the Hub address, then routes session and workspace requests against it. Use the client to skip implementing discovery and auth yourself.
</Callout>
...
| `region` | `string` | `AGENTUITY_REGION` or `usc` | Region used for Hub URL discovery |
| `orgId` | `string` | - | Organization ID for multi-tenant operations (optional) |
| `logger` | `Logger` | Minimal logger | Custom logger instance |

<Callout type="tip" title="Full Coder Reference">
For every `CoderClient` method with signatures, parameters, and return types, see [CoderClient Reference](/reference/sdk-reference/coder). For concepts, session lifecycle, and workflow guidance, see [Coder](/services/coder).
</Callout>
