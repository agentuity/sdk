...
The project metadata and `.env` are written inside `apps/web`. A monorepo-aware build can still stage the deployment bundle from the workspace root and set `workingDirectory` in `launch.json`.
...
```bash
npx @agentuity/migrate@beta --help
```
