...
- **Cowork / cloud sandbox** (`agentuity auth whoami` fails, or you're in a `/sessions/` path):
  ...
  **STOP. Do NOT create files in the sandbox VM.** The sandbox filesystem is ephemeral — the user will never see those files. Their real filesystem is mounted under `/sessions/*/mnt/`. Find it and work there:
  ...
  agentuity create my-app --yes --dir /sessions/<session>/mnt/<workspace>/my-app
  ```
...
- **New project (no existing code):** Use `agentuity create my-app` — see "Creating a New Project" below.
...
This scaffolds a full Agentuity project with the correct structure, dependencies, and config. Only use this for brand-new projects.

**Skip prompts:** Add `--yes` (or `--force`) to skip confirmation prompts:
```bash
agentuity create my-app --yes
```
...
The dev server runs at **http://localhost:3500** by default. Always read the actual output for the URL.
...
├── agentuity.config.ts   # Build config (REQUIRED for project import — can be empty)
├── app.ts                # App entry point (REQUIRED)
├── tsconfig.json         # TypeScript config (REQUIRED — must include @agent/* path)
├── .env                  # Environment variables (auto-loaded)
├── package.json
└── src/
    ├── agent/<name>/     # AI agent handlers (auto-discovered by folder name)
    │   └── index.ts
...
- **`app.ts`** — App entry point. Minimal example:
  ```typescript
  import { createApp } from '@agentuity/runtime';
  const { server, logger } = await createApp();
  logger.debug('Running %s', server.url);
  ```
  If you have API routes, mount them with setup/shutdown hooks:
  ```typescript
  import { createApp } from '@agentuity/runtime';
  ...
  const { server, logger } = await createApp({
    ...
    setup: async () => {
      // Return value is available as ctx.app — initialize global resources here
    },
    shutdown: async (_state) => {
      // State is whatever setup() returned — close resources here
    },
  });
...
- **`agentuity.config.ts`** — Build configuration. **Required** — the CLI needs this file to recognize the directory as a valid Agentuity project. Can start minimal and add options as needed. Use `satisfies AgentuityConfig` for type safety:
  ```typescript
  ...
  ```
  - Docs: https://agentuity.dev/reference/cli/build-configuration.md

**Key points:**

- **Agents are auto-discovered** from `src/agent/` — add a folder with an `index.ts` file and it's registered automatically. The folder name becomes the agent name.
...
- **To expose an agent over HTTP**, create an API route that calls it. Use `agent.validator()` for automatic schema validation:
  ...
  // Route paths are relative to the mount point in app.ts
  // If mounted at '/api', this becomes /api/weather
  router.post('/weather', weather.validator(), async (c) => {
  ...
  ```
  Then mount the router in `app.ts` at `/api`:
  ```typescript
  router: [{ path: '/api', router: api }]
  ```
  Access it at `http://localhost:3500/api/weather`.
  
  **Do NOT duplicate the `/api` prefix** — if mounted at `/api`, define routes as `/weather`, not `/api/weather`.
...
- **Routes use Hono** (not Express) — create routers with `createRouter` from `@agentuity/runtime`.
...
| --- | --- | --- |
| Express/Fastify routes | `src/api/index.ts` | Rewrite routes using Hono via `createRouter` from `@agentuity/runtime` |
...
3. **Create `app.ts`** in the project root — this is REQUIRED:
   ```typescript
   import { createApp } from '@agentuity/runtime';
   import api from './src/api/index';
   ...
   logger.debug('Running %s', server.url);
   ```
   If there are no API routes, omit the `router` option.
...
5. **Convert API routes** to Hono in `src/api/index.ts`:
   ...
   // Routes are relative to mount point — mounted at '/api' in app.ts
...
7. **Move environment variables** into `.env` (no `dotenv` import needed)
8. **Update `package.json`** — add required scripts and dependencies:
   ```json
   {
     "scripts": {
...
9. **Create `agentuity.config.ts`** in the project root (can be empty — the CLI requires this file to recognize the project):
   ```typescript
   export default {};
   ```
...
- **Hono for HTTP routing** — use `createRouter` from `@agentuity/runtime`
- **File-based agent discovery** — folder name under `src/agent/` = agent name
...
- **AI Gateway** — LLM requests (OpenAI, Anthropic, etc.) automatically route through Agentuity's AI Gateway using the SDK key. **No separate API keys needed.** Just use the AI SDK (`ai` package) normally — it works out of the box.
- **Workbench** — Add `workbench: { route: '/workbench' }` to `agentuity.config.ts` to get a built-in UI for testing agents at `http://localhost:3500/workbench`. See https://agentuity.dev/agents/workbench.md
...
| Empty `agentuity.config.ts` without types | Use `satisfies AgentuityConfig` from `@agentuity/cli` | Type-safe config catches errors early |
...
| Express-style routing | Use Hono via `createRouter` | Agentuity uses Hono, not Express |
...
| Putting agents outside `src/agent/` | Use `src/agent/<name>/index.ts` | Auto-discovery requires this path |
...
| Running `project import` without `agentuity.config.ts` | Create `agentuity.config.ts` (even empty: `export default {}`) before importing | CLI requires this file to recognize the directory as a valid Agentuity project |
