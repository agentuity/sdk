...
Agent models are configured in `~/.config/opencode/opencode.json`. OpenCode reads plugin-provided agents and the `agent` entries in `opencode.json`, so you can override any Agentuity Coder agent by display name.
