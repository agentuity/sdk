...
if (!baseUrl) {
  throw new Error('AGENTUITY_TRANSPORT_URL environment variable is required');
}
...
<Callout type="info" title="Use the CLI for one-off queue operations">
For one-off queue management, use `agentuity cloud queue --help` to inspect the current queue commands and flags. The service page covers the queue lifecycle and links to the lower-level API reference.
</Callout>

## Alternative: Hono Routes

If you want to centralize storage logic in a Hono app, use the Agentuity middleware and keep service access behind routes.
...
type Variables = Pick<Services, 'kv' | 'vector' | 'stream' | 'logger'>;
...
type SessionPatch = z.infer<typeof SessionPatchSchema>;
...
type Variables = Pick<Services, 'kv' | 'logger'>;
...
Convert Zod schemas to JSON Schema for tooling or OpenAPI generation:
...
- **Manual configuration** with `@agentuity/server` is for custom adapters, direct service URL overrides, or lower-level API helpers
- **Required env vars**: standalone clients read `AGENTUITY_SDK_KEY` by default; manual `@agentuity/server` setup also needs the service base URL or region it will target
- **HTTP routes** are an alternative for centralized logic, middleware, or sharing across apps
- **Works with any framework**: Next.js, React Router, Remix, Express, and more.

## See Also

- [Standalone Packages](/reference/standalone-packages): Full list of service packages and their APIs
- [Queues](/services/queues): queue concepts, publish options, lifecycle, and API links
- [Message Queues API](/reference/api/queues): lower-level queue, message, destination, source, and dead letter queue endpoints
- [Using Hono with Agentuity](/frameworks/hono): Route creation and middleware patterns
- [Hono RPC + TanStack](/cookbook/patterns/hono-rpc-tanstack-query): Share Hono route types with a React client
