...
After `bun create agentuity@beta`, expect the framework's normal files plus:

| Addition | Where | When |
|----------|-------|------|
| `@agentuity/cli@beta` | `package.json` devDependencies | Always |
...
React Router, Vite + React, and TanStack Start are existing-app adoption paths today. See [Frameworks](/frameworks) for adding Agentuity to those apps without using `bun create agentuity@beta`.
