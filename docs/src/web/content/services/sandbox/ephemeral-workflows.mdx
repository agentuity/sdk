...
See [Sandbox SDK Usage](/services/sandbox/sdk-usage) for file operations, streaming output, pause/resume, and snapshots. See [Coding agents in sandboxes](/services/sandbox/coding-agents) when the workflow needs a tool image such as `opencode:latest` instead of a plain language runtime.
