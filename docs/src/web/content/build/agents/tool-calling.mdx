...
| Use | Good fit |
|-----|----------|
| [AI Gateway structured output](/services/ai-gateway#structured-output) | typed JSON output, provider-agnostic model routing, and app-owned validation |
| [AI SDK](#ai-sdk) | automatic tool loops, AI SDK UI streams, or an app already built around AI SDK primitives |
| [OpenAI Responses API](#openai-responses-api) | apps already on Responses with `previous_response_id` turn chaining or Responses-only tools |
| [Anthropic Messages API](#anthropic-messages-api) | apps that need Anthropic `tool_use` block fidelity or provider-specific schema controls |
...
This section uses the Anthropic AI SDK provider because AI SDK owns the loop. Under `agentuity dev`, `ANTHROPIC_API_KEY` and `ANTHROPIC_BASE_URL` can route through AI Gateway from `AGENTUITY_SDK_KEY` when no provider key override is set. For structured JSON without an AI SDK loop, use [`AIGatewayClient.completeStructured()`](/services/ai-gateway#structured-output).
