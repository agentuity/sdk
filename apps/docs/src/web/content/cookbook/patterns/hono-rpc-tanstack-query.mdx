...
Use `import type` in frontend code. Type-only imports are erased from the JavaScript bundle:

```typescript title="src/web/api.ts"
import { hc } from 'hono/client';
import type { UsersRoute } from '../api/users';

export const client = hc<UsersRoute>('/api');
```

For larger apps, you can keep a dedicated type-only barrel:
...
// Imported routers use paths relative to these mount points
const router = new Hono<Env>()
   .route('/users', users)
   .route('/posts', posts);
