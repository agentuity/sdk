...
```bash
bun add hono ai @ai-sdk/openai @agentuity/vector@beta zod
```
...
export async function recommendProduct(body: unknown): Promise<Recommendation> {
  ...
  if (!model) {
    throw new Error('Set OPENAI_MODEL to the recommendation model this route should use.');
  }
...
```bash
curl 'http://localhost:3000/api/products/search?q=comfortable%20office%20chair&maxPrice=300'
```
