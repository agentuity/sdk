...
Coder has three related but distinct selection layers:
...
Use `enabledAgents` to choose from the roster your Hub exposes for a session or workspace. Use `defaultAgent` when you want routed work to prefer a specific agent.

Workspaces bundle the setup a coding session needs: repositories, saved skills, skill buckets, dependency setup, Lead prompt guidance, and agent selections. Custom agents let you package reusable domain knowledge, tools, service access, and companion agents. Use `createAgentBuilderSession()` when you want Agent Builder to help create or update a reusable custom agent programmatically.
...
```bash
bun add @agentuity/coder@beta @agentuity/telemetry@beta
```
...
Custom agents package reusable domain knowledge, tools, and service access that can be attached to sessions. Use `createAgentBuilderSession()` to let Agent Builder help author or update one interactively.
...
- [Choosing Coder Agents for a Session](/cookbook/patterns/choosing-built-in-agents-for-a-coder-session): Choose the Hub-provided roster for a session
