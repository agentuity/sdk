...
| `durationMs` | `number` | Execution duration in milliseconds, when available |
| `stdoutStreamUrl` | `string` | URL to fetch stdout stream |
| `stderrStreamUrl` | `string` | URL to fetch stderr stream |
| `outputTruncated` | `boolean` | Whether captured output was truncated |
| `autoResumed` | `boolean` | Whether the sandbox resumed automatically before execution |
