...
/**
 * Resolve the runtime path to the Agentuity public-asset-path lint plugin so
 * the wrapper config below can import it from inside the user's project.
 *
 * In workspace/dev contexts we read it from the CLI source tree; when the
 * CLI is installed as a published package (bundled via `bun build --target
 * bun`) we import the plugin directly by package name.
 */
function resolveLintPluginImport(): string {
	// Walk up from this file's directory looking for the source plugin. In the
	// bundled CLI this file IS the bundle, and the relative path below won't
	// resolve — we fall back to the package name which Bun will resolve via
	// the CLI's own node_modules / workspace link.
	const here = new URL('.', import.meta.url).pathname;
	const candidate = join(here, 'public-asset-path-plugin.ts');
	if (existsSync(candidate)) {
		return candidate;
	}
	// Fallback: a compiled .js next to this file.
	const compiled = join(here, 'public-asset-path-plugin.js');
	if (existsSync(compiled)) {
		return compiled;
	}
	// Last resort: resolve via the CLI package. This works when the CLI is
	// installed and the wrapper config runs under `bun x vite` from the user's
	// project root.
	return '@agentuity/cli/build/public-asset-path-plugin';
}

/**
 * Generate a wrapper vite config that re-exports the user's config with the
 * Agentuity lint plugin appended. Written to `.agentuity/vite.client.config.ts`
 * and passed to the `vite build` subprocess via `--config`.
 *
 * The wrapper is regenerated on every build so it stays in sync with both the
 * CLI's plugin location and the user's config path.
 */
async function writeClientWrapperConfig(rootDir: string, userConfigPath: string): Promise<string> {
	const wrapperDir = join(rootDir, '.agentuity');
	mkdirSync(wrapperDir, { recursive: true });
	const wrapperPath = join(wrapperDir, 'vite.client.config.ts');

	const pluginImport = resolveLintPluginImport();

	// Vite configs can be objects, functions, or promises returning either.
	// `mergeConfig` normalises all three while preserving plugin order from the
	// user's config, so framework plugins (React/Svelte/Vue) still run first.
	const wrapperSource = `// AUTO-GENERATED by @agentuity/cli on every client build. Do not edit.
// Wraps the project's vite.config.ts to inject Agentuity's lint plugin while
// preserving subprocess-isolated plugin loading (see commit 2481bea4).
import { defineConfig, mergeConfig, type UserConfig, type UserConfigFn } from 'vite';
import userConfigImport from ${JSON.stringify(userConfigPath)};
import { publicAssetPathPlugin } from ${JSON.stringify(pluginImport)};

export default defineConfig(async (env) => {
	const raw = userConfigImport as UserConfig | UserConfigFn | Promise<UserConfig>;
...
`;
		...
		// Generate a wrapper config that re-exports the user's config and appends
		// the Agentuity lint plugin. The subprocess loads THIS config via --config,
		// which preserves subprocess isolation (no Bun/Vite module-loader regression
		// — see commit 2481bea4) while still giving us a way to enforce asset-path
		// conventions on the user's source.
		const wrapperConfigPath = await writeClientWrapperConfig(rootDir, viteConfigPath);
