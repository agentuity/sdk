...
Use `new Hono<Env>()` when your middleware only reads built-in Agentuity context, like `c.var.logger`. If middleware stores custom values with `c.set()`, add those values to Hono's `Variables` type so downstream handlers can read them from `c.var`.
...
const apiKeyAuth = createMiddleware(async (c, next) => {
  ...
  const keyData = await c.var.kv.get<ApiKeyRecord>('api-keys', apiKey);
  if (!keyData.exists) {
