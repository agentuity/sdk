...
setTracingDisabled(true); // [!code highlight]
...
<Callout type="info" title="Disable OpenAI tracing">
Call `setTracingDisabled(true)` when running on Agentuity. Agentuity's built-in [observability](/services/observability/tracing) captures traces, logs, and metrics automatically.
</Callout>
