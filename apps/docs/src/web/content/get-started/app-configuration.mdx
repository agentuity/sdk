...
addEventListener('agent.started', (event, agent, ctx) => {
  logger.info('Agent started', { name: agent.metadata.name });
});

addEventListener('agent.completed', (event, agent, ctx) => {
  logger.info('Agent completed', { session: ctx.sessionId });
});
```
...
Use build configuration to add Vite plugins (like Tailwind CSS), define build-time constants, or bundle extra files (data, templates, models) into your deployment.
</Callout>
...
For generated environment types, later files override earlier ones. That means `.env` currently wins over `.env.development` if both define the same key during local development.
...
Instead of commenting out lines when switching contexts, put shared defaults in `.env` and values that only exist in local development in `.env.development`. Be careful with duplicate keys: today the CLI reads `.env.development` before `.env`, so `.env` wins when both define the same variable.
</Callout>
