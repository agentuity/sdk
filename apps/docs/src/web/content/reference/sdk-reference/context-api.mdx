...
interface AgentContext<TConfig = unknown, TAppState = Record<string, never>> {
  ...
  app: TAppState;                     // App-level state when your runtime wiring provides it
...
**Identifiers:**
- `sessionId`: Unique identifier for the current request. Sub-agent calls within that same request share it.
...
- `app`: App-level shared state if your runtime wiring provides it. For new code, prefer module-scoped shared dependencies plus `registerShutdownHook()` rather than centering examples on `ctx.app`.
...
`ctx.config` is the stable, documented place for agent-local setup results. The runtime types still include `ctx.app`, but the current `createApp()` API is best documented with module-scoped shared dependencies plus `registerShutdownHook()` for cleanup rather than new examples built around app-level setup state.
</Callout>
