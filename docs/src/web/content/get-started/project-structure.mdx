...
React Router, Vite + React, and TanStack Start are existing-app adoption paths today. See [Frameworks](/frameworks) for adding Agentuity to those apps without using the Agentuity create flow.
...
To keep sibling apps, docs, and other non-runtime paths out of the monorepo deploy bundle, add a `.agentuityignore` file at the workspace root or in the app package (gitignore syntax; paths are relative to the monorepo root). See [Monorepo staging and `.agentuityignore`](/reference/cli/deployment#monorepo-staging-and-agentuityignore) and [Deploy Framework Apps](/deploy-operate/deploy-framework-apps#exclude-monorepo-paths-from-the-deploy-bundle).
