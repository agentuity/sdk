...
import {
	generateWorkbenchMainTsx,
	generateWorkbenchIndexHtml,
	generateWorkbenchStylesCss,
} from './workbench';
	...
	if (existsSync(appFile) && workbench && workbench.hasWorkbench) {
		...
		try {
			...
			if (existsSync(workbenchStylesPath)) {
				...
				logger.warn('Workbench dist/standalone.css not found, using generated CSS with import');
			}
