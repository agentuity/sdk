...
agentuity cloud session list --deployment-id=deploy_abc123xyz
...
List and manage conversation threads. For details on how threads work in agents, see [State Management](/services/storage/key-value).
...
All commands support `--json` for machine-readable output. Combined with a JSON processor such as `jq`, you can filter and transform results:
