...
Coder has HTTP endpoints, but no single static base URL. Clients first call the Agentuity API to discover your org's Hub address, then route session and workspace requests to that Hub. [`@agentuity/coder`](/reference/standalone-packages#coder) and `agentuity coder` handle discovery, auth, and routing for you.
</Callout>
...
const client = new CoderClient({
  url: 'https://coder.example.com',     // Override Hub URL (skips discovery)
  apiKey: 'agentuity_sk_...',           // Override AGENTUITY_SDK_KEY
  region: 'usc',                        // Region for Hub URL discovery
...
- [`@agentuity/coder` Package](/reference/standalone-packages#coder): Standalone package reference
