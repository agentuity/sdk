...
description: Collected logs and OpenTelemetry tracing via ctx.logger and ctx.tracer
---

Collected logs via `ctx.logger` and distributed tracing via `ctx.tracer` (OpenTelemetry), available in agents, routes, and standalone scripts.

## Logging

Use `ctx.logger` to emit log lines that the platform captures and displays in the Console. Use child loggers when you want fields attached as log attributes.
...
In route handlers, the logger is available on `c.var.logger` instead of `ctx.logger`. The API is identical.
