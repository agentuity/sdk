...
function App() {
	...
	const historyQuery = useQuery({
		...
		queryFn: () => fetchJson<HistoryData>('/api/translate'),
	});

	const mutation = useMutation({
		mutationFn: translateText,
		onSuccess: (data) => {
			queryClient.setQueryData(['translation-history'], data);
		},
	});

	const clearMutation = useMutation({
		mutationFn: () => fetchJson<HistoryData>('/api/translate', { method: 'DELETE' }),
		onSuccess: (data) => {
			mutation.reset();
			queryClient.setQueryData(['translation-history'], data);
		},
	...
	const historyData = result ?? historyQuery.data;
			...
			<div className="flex w-full max-w-3xl flex-col gap-4 p-16">
				...
				<div className="flex flex-col gap-6 rounded-lg border border-gray-900 bg-black p-8">
					<div className="flex items-center justify-between">
						...
						{history.length > 0 && (
							<button
								className="rounded border border-gray-900 bg-transparent px-3 py-1.5 text-xs text-gray-500 transition-all duration-200 hover:border-gray-700 hover:bg-gray-900 hover:text-white disabled:opacity-50"
