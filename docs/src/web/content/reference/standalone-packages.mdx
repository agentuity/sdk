...
| Package | Install | Use |
|---------|---------|-----|
| [`@agentuity/keyvalue`](#key-value-storage) | `npm install @agentuity/keyvalue` | exact-key data, TTL-backed cache, preferences, counters |
| [`@agentuity/vector`](#vector-search) | `npm install @agentuity/vector` | documents, embeddings, semantic search, retrieval |
| [`@agentuity/storage`](#object-storage) | `npm install @agentuity/storage` | S3-compatible file storage under Bun or Node.js |
| [`@agentuity/stream`](#durable-streams) | `npm install @agentuity/stream` | durable, resumable streams and generated files |
| [`@agentuity/aigateway`](#ai-gateway) | `npm install @agentuity/aigateway` | model catalog lookup and routed model requests |
| [`@agentuity/queue`](#message-queues) | `npm install @agentuity/queue` | async handoff to worker or pub/sub queues |
| [`@agentuity/email`](#email) | `npm install @agentuity/email` | managed email addresses, outbound email, inbound records |
| [`@agentuity/schedule`](#schedules) | `npm install @agentuity/schedule` | cron schedules and delivery tracking |
| [`@agentuity/task`](#tasks) | `npm install @agentuity/task` | work items, comments, tags, attachments, activity |
| [`@agentuity/sandbox`](#sandbox) | `npm install @agentuity/sandbox` | isolated command execution and long-lived sandboxes |
| [`@agentuity/webhook`](#webhooks) | `npm install @agentuity/webhook` | managed webhook ingest URLs, destinations, receipts |
| [`@agentuity/coder`](#coder) | `npm install @agentuity/coder` | Coder sessions, workspaces, custom agents, replay data |
| [`@agentuity/db`](#database) | `npm install @agentuity/db` | trusted admin scripts against the database service API |
| [`@agentuity/hono`](#hono) | `npm install @agentuity/hono hono` | Hono middleware that injects common clients on `c.var.*` |
| [`@agentuity/telemetry`](#telemetry) | `npm install @agentuity/telemetry` | logger, tracer, and meter setup outside Hono |
