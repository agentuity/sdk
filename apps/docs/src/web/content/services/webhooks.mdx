...
|--------|--------|---------|
| SDK client | `WebhookClient` from `@agentuity/webhook` | Client-style management for common operations |
| Server utilities | `@agentuity/server` | Function-style management, including destination updates |
...
```typescript
import { APIClient, createLogger, getAPIBaseURL } from '@agentuity/server';

const logger = createLogger();
const client = new APIClient(getAPIBaseURL(), logger);
```

The examples below use `@agentuity/server` function exports because they expose the full management surface, including destination updates. For client-style usage, instantiate `WebhookClient` from `@agentuity/webhook`; it wraps the same service for common operations.
...
```typescript
import { APIClient, createLogger, createWebhook, getAPIBaseURL } from '@agentuity/server';
