...
See [Build Agents](/build/agents) for model-backed workflows and [Background Work](/build/background-work) for moving slower work out of request handlers.
</Callout>
...
If a legacy app still uses `createAgent()` from `@agentuity/runtime`, treat direct agent imports and `.run()` calls as migration context. New v3 code should use framework routes, shared functions, and standalone service clients.
