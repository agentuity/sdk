...
Project config can also be profile-specific: when profile `staging` is active, the CLI prefers `agentuity.staging.json` over `agentuity.json` if that file exists.

For explicit multi-environment deploys without switching profiles, use `--project-config` and global `--env` instead:
...
See [Multiple environments](/reference/cli/deployment#multiple-environments).
