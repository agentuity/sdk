...
console.log(recentExports.total);
...
See [Background Work](/build/background-work): it pairs queues, KV status records, and durable streams for jobs that should continue after a request returns.
...
- [Background Work](/build/background-work): run long jobs with queues, status records, and streams
