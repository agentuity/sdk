...
async function main() {
	...
	console.log(`✅ All ${NUM_CONCURRENT} processes completed (exit codes don't matter for this test)`);
	console.log();
	...
	if (sessions.length !== NUM_CONCURRENT) {
		...
		console.error('   This might indicate session directories were deleted by concurrent processes!');
		process.exit(1);
	}
	...
	writeFileSync(join(oldDirPath, 'session.json'), JSON.stringify({ sessionId: oldDirName, bucket: oldBucket }));
