...
Create an OpenAI agent and wrap it with [`createAgent()`](/patterns/agents-as-a-pattern):
...
The OpenAI Agents SDK enables tracing by default in server runtimes. If Agentuity should be your trace source, set `OPENAI_AGENTS_DISABLE_TRACING=1` or call `setTracingDisabled(true)` during process startup. If you instantiate `Runner` directly instead of using the `run()` helper, pass `new Runner({ tracingDisabled: true })`.
</Callout>
...
function createTriageAgent(onRefundEscalation: (reason: string) => void) {
  return Agent.create({ // [!code highlight]
    name: 'Triage Agent',
    ...
    handoffs: [
      ...
      handoff(refundAgent, { // [!code highlight]
        onHandoff: (_ctx, input) => {
          if (input?.reason) {
            onRefundEscalation(input.reason);
          }
        },
...
- [Creating Agents](/patterns/agents-as-a-pattern): Agentuity agent patterns and schemas
- [AI Gateway](/services/ai-gateway): Managed model credentials across providers
