...
export const MachineSchema = z.object({
	...
	privateIPv4: z.string().nullable().optional().describe('Private IPv4 address of the machine.'),
	availabilityZone: z.string().nullable().optional().describe('Cloud provider availability zone.'),
	instanceType: z.string().nullable().optional().describe('Cloud provider instance type (e.g., n1-standard-2).'),
	instanceTags: z.array(z.string()).nullable().optional().describe('Tags assigned to the cloud instance.'),
	deploymentCount: z.number().optional().describe('Number of active deployments on this machine.'),
	status: z.string().describe('Current status of the machine (e.g., running, stopped, errored).'),
	provider: z.string().describe('Cloud provider (e.g., gcp, aws).'),
	region: z.string().describe('Cloud region where the machine is deployed.'),
	startedAt: z.string().nullable().optional().describe('ISO 8601 timestamp when the machine was started.'),
	stoppedAt: z.string().nullable().optional().describe('ISO 8601 timestamp when the machine was stopped.'),
	pausedAt: z.string().nullable().optional().describe('ISO 8601 timestamp when the machine was paused.'),
	erroredAt: z.string().nullable().optional().describe('ISO 8601 timestamp when the machine entered error state.'),
	error: z.string().nullable().optional().describe('Error message if the machine is in an error state.'),
	orgId: z.string().nullable().optional().describe('Organization ID that owns this machine.'),
	orgName: z.string().nullable().optional().describe('Organization name that owns this machine.'),
	createdAt: z.string().describe('ISO 8601 timestamp when the machine was created.'),
	updatedAt: z.string().nullable().optional().describe('ISO 8601 timestamp when the machine was last updated.'),
	metadata: z.record(z.string(), z.unknown()).nullable().optional().describe('Arbitrary metadata associated with the machine.'),
...
export const MachineDeploymentSchema = z.object({
	...
	project: MachineDeploymentProjectSchema.nullable().optional().describe('Project associated with this deployment.'),
	resources: MachineDeploymentResourcesSchema.nullable().optional().describe('Resource allocation for this deployment.'),
...
export const OrgAuthStatusResponseSchema = APIResponseSchema(
	z.object({
		publicKey: z.string().nullable().describe('Public key for the organization, or null if not enrolled.'),
	})
);
