...
Add threaded comments to any task. `userId` identifies the commenter; `author` is an optional `{ id, name }` reference that adds a display name.
...
<Callout type="warning" title="Task attachments need cloud-backed storage">
Local task storage supports tasks, comments, tags, and changelog data. Attachment methods throw `AttachmentNotSupportedError` in local storage.
</Callout>
...
| `created_id` | Filter by creator user ID |
| `parent_id` | Filter by parent task (returns subtasks) |
| `project_id` | Filter by project ID |
| `tag_id` | Filter by a specific tag |
| `include` | Include additional fields such as `description`, `metadata`, `tags`, `subtask_count`, `created_id`, or `deleted` |
...
Set `parent_id` to build epic-to-feature-to-task hierarchies. Query subtasks with `list({ parent_id: epicId })`. Pass `include: ['subtask_count']` when you need direct child counts without fetching every subtask.
...
- **Page results with `limit` and `offset`**: Set an explicit `limit` when listing tasks in agents so pagination stays predictable.
