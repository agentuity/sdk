...
function generateAgentRegistry(
	srcDir: string,
	agentInfo: Array<Record<string, string>>,
	generatedDir?: string
) {
...
`;
	...
	if (generatedDir) {
		...
		// Generate imports with paths relative to generatedDir (assumed to be in src/)
		const srcImports = agentInfo
			.map(({ name, path }) => {
				const camelName = toCamelCase(name);
				// Path is like ./agent/hello, we need ../agent/hello (from src/_generated/)
...
`;
		...
		const srcRegistryPath = join(resolvedGeneratedDir, 'registry.generated.ts');
...
const AgentuityBundler: BunPlugin = {
	...
	setup(build) {
		...
		const generatedDir = build.config.define?.['process.env.AGENTUITY_GENERATED_DIR']
			? JSON.parse(build.config.define['process.env.AGENTUITY_GENERATED_DIR'])
			: undefined;
...
(() => {
	...
	router.get('/*', async (c, next) => {
		...
		return c.html(index);
				...
				generateAgentRegistry(srcDir, agentInfo, generatedDir);
				...
				if (routeInfoList.length > 0) {
					...
					generateRouteRegistry(srcDir, routeInfoList, generatedDir);
				}
