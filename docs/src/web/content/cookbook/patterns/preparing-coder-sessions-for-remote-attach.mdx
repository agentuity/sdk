...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.error('Failed to list reconnectable sessions');
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (session.historyOnly) {
      ...
      logger.info('Session is history-only. Use replay or session detail instead of re-attaching.');
      return;
    }

    if (session.runtimeAvailable) {
      // Attaching before the runtime is up fails; this check confirms it's ready
      logger.info('Session is attachable.');
      logger.info(
        JSON.stringify(
          {
            sessionId: session.sessionId,
    ...
    logger.info('Session is not attachable yet.');
    logger.info(
      JSON.stringify(
        {
          sessionId: session.sessionId,
    ...
    const message = error instanceof Error ? error.stack ?? error.message : String(error);
    logger.error('Failed to prepare the session for remote attach');
    logger.error(message);
    process.exitCode = 1;
  }
}
