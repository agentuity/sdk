...
Method reference for `CoderClient` from the standalone [`@agentuity/coder`](/reference/standalone-packages#coder) package. Coder is not a `ctx.*` service; use the client from framework routes, scripts, workers, or external apps.
...
logger.info('Coder session created', session.sessionId);
```
...
```typescript
const workspace = await client.refreshWorkspaceSnapshot('ws_abc123');
```

### validateWorkspaceDependencies(dependencies)
