---
title: Agent Creation and Handlers (v2 Compatibility)
short_title: Agents
description: Compatibility reference for v2 runtime createAgent() handlers
---

This page documents v2 runtime agents created with `createAgent()` from `@agentuity/runtime`. In v3 apps, start with [Agents as a Pattern](/patterns/agents-as-a-pattern), [Frameworks](/frameworks), and [Runtime to Frameworks](/migration/runtime-to-frameworks): model agent-like work as framework routes, shared functions, workers, or standalone service clients.

<Callout type="info" title="Keep createAgent() for existing runtime apps">
The `createAgent()` examples below still work for existing v2 runtime apps. New v3 code usually keeps model-backed work in plain functions and calls it from framework routes or workers.
</Callout>

In the v2 runtime model, agents are the core building blocks of an Agentuity application. Use `createAgent()` to define typed handlers with schema validation, lifecycle hooks, and route middleware.
...
- `config`: Configuration object with the following properties:
  ...
  - `setup` (optional): Async function called once on app startup. Return agent-specific config for `ctx.config`
  - `shutdown` (optional): Async cleanup function called on app shutdown with the setup result

**Return Value**

Returns an `AgentRunner` object with the following properties:
- `metadata`: Agent metadata (`id`, `agentId`, `filename`, `version`, `name`, `description`)
- `run(input)`: Execute the agent. Input is required when the agent has an input schema, omitted otherwise.
...
**Note**: To call agents from other agents, import them directly: `import otherAgent from '@agent/other/agent'; otherAgent.run(input)` (see [Agent Communication](/reference/sdk-reference/communication)).
...
The `setup` function runs once when the app starts and returns agent-specific configuration available via `ctx.config`. This is useful for:
...
For the v3 pattern, see [Agents as a Pattern](/patterns/agents-as-a-pattern). For older runtime projects, the examples on this page remain the compatibility reference.
...
For streaming patterns with Hono routes, see [Using Hono with Agentuity](/frameworks/hono).
...
Use `stream()` when the route builds the `ReadableStream` itself. For validated non-agent streams, use the standalone `validator({ input, stream: true })` helper from `@agentuity/runtime`.
...
For streaming patterns, chunked responses, and AI SDK integration, see [Streaming Responses](/patterns/chat-and-streaming).
