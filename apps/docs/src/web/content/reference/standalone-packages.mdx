...
Manage Coder Hub sessions, participants, replay data, and loop state.
...
|--------|------|---------|-------------|
| `apiKey` | `string` | `AGENTUITY_SDK_KEY` env var | API key for authentication |
| `url` | `string` | Discovered via Catalyst | Base URL for the Coder API |
| `orgId` | `string` | - | Organization ID for multi-tenant operations (optional) |
| `logger` | `Logger` | Minimal logger | Custom logger instance |

<Callout type="tip" title="Full Coder Reference">
For the complete Coder API including workspaces, skill library, and GitHub integration, see [Coder](/services/coder).
</Callout>
