...
When deployed on Agentuity, Mastra's `openai/gpt-4o-mini` model string routes through the [AI Gateway](/agents/ai-gateway) automatically. Set `OPENAI_BASE_URL` to point at the gateway, and Mastra uses it without code changes. See the [gateway bridge](#ai-gateway-bridge) section below.
</Callout>
