...
async function main() {
	...
	try {
		...
		// Best-effort cleanup: soft-delete every task we created. Any
		// associated comments / attachments come along for the ride.
		if (createdTaskIds.length > 0) {
			...
			for (const id of createdTaskIds) {
				try {
					...
					console.log(`   ⚠️  Could not delete task ${id}: ${msg}`);
				}
			}
		}
	}
