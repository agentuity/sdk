...
Use Agentuity services in external apps, scripts, or backends. Standalone packages provide the simplest setup, or use `@agentuity/server` directly for custom configurations.

## Standalone Packages

The fastest way to access Agentuity services from external apps. Each package handles authentication and configuration automatically.

<Callout type="info" title="Environment Variables">
Set `AGENTUITY_SDK_KEY` and `AGENTUITY_REGION` in your `.env` file. See [Standalone Packages](/reference/standalone-packages#configuration) for details.
</Callout>
