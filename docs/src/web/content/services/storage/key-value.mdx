...
`search()` returns a plain `Record<string, KeyValueItemWithMetadata<T>>`, not a `Map`. Iterate with `Object.entries`. Each entry includes the typed value plus `size`, `expiresAt`, and access timestamps.
