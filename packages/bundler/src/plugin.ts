...
function generateAgentRegistry(srcDir: string, agentInfo: Array<{ name: string; path: string }>) {
	const imports = agentInfo
		.map(({ name, path }) => {
			const camelName = toCamelCase(name);
			return `import ${camelName}Agent from '${path.replace('../agents/', './')}';`;
		})
		.join('\n');
...
${agentInfo
	.map(({ name, path }) => {
		const camelName = toCamelCase(name);
		return `import type ${camelName}Agent from '${path.replace('../agents/', './')}';`;
	})
	.join('\n')}
