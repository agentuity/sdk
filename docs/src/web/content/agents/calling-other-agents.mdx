...
Agent imports only work within the same project. To call an agent in another project or organization, expose it through an HTTP route and call that route with `fetch()`.
</Callout>

Call the deployed route using standard HTTP:

```typescript
import { createAgent } from '@agentuity/runtime';
import { s } from '@agentuity/schema';

const ExternalResponse = s.object({ response: s.string() });
...
const agent = createAgent('External Caller', {
  ...
  handler: async (ctx, input) => {
    try {
      const routeUrl = process.env.EXTERNAL_AGENT_ROUTE_URL;
      if (!routeUrl) {
        throw new Error('EXTERNAL_AGENT_ROUTE_URL is required');
      }
      ...
      if (!response.ok) {
        throw new Error(`External agent route returned ${response.status}`);
      }

      const data = ExternalResponse.parse(await response.json());
      return { result: data.response };
    } catch (error) {
      ctx.logger.error('External agent call failed', { error });
      throw error;
    }
  },
});
...
const consumer = createAgent('Consumer', {
  ...
  handler: async (_ctx, input) => {
    const result = await source.run({ id: input.id });
...
- [LLM as a Judge](/cookbook/patterns/llm-as-a-judge): Add quality checks to your agent workflows
