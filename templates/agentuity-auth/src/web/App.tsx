...
import { UserButton, OrganizationSwitcher, SignedIn, SignedOut } from '@daveyplate/better-auth-ui';
			...
			<div className="flex flex-col gap-8 max-w-3xl w-full p-16">
				...
				<div className="bg-card border border-border rounded-lg p-6">
					<div className="flex items-center justify-between mb-4">
						<h3 className="text-foreground text-lg font-normal m-0">🔐 Authentication</h3>
						<SignedIn>
					...
					<SignedIn>
						<div className="flex flex-wrap gap-2 mt-4">
							<a
								href="/account"
								className="text-xs bg-secondary hover:bg-accent border border-border rounded px-3 py-2 text-secondary-foreground no-underline"
							>
								Account Settings
							</a>
							<a
					...
					<SignedOut>
						<div className="flex items-center gap-4">
							<p className="text-muted-foreground text-sm m-0">Not signed in</p>
							<a
								href="/auth/sign-in"
								className="text-xs bg-primary text-primary-foreground rounded px-4 py-2 no-underline"
							>
								Sign In
							</a>
							<a
								href="/auth/sign-up"
								className="text-xs bg-secondary hover:bg-accent border border-border rounded px-4 py-2 text-secondary-foreground no-underline"
							>
								Sign Up
							</a>
						</div>
					...
					<div className="flex flex-col gap-6">
						{[
							...
							},
							{
								...
								text: (
									<>
										Add <code className="text-white">authMiddleware</code> to routes in{' '}
										<code className="text-white">src/api/index.ts</code> to require
										authentication.
									</>
								),
							},
...
export function App() {
	...
	// Route /auth/* paths to BetterAuth UI (sign-in, sign-up, etc.)
	if (pathname === '/auth' || pathname.startsWith('/auth/')) {
		return <AuthPages />;
	}

	// Route /account/* paths to BetterAuth UI (settings, security, api-keys)
	if (pathname === '/account' || pathname.startsWith('/account/')) {
		return <AuthPages />;
	}

	// Route /organization/* paths to BetterAuth UI (org settings)
