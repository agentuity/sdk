...
Managed cloud KV clamps per-key TTL values passed to `set()` that are less than 60 seconds to 60 seconds, and greater than 365 days to 365 days. Namespace defaults passed to `createNamespace()` must be `0` or between 60 seconds and 365 days. Out-of-range namespace defaults return a validation error. A TTL of `0` means the key, or namespace default, never expires.
</Callout>
