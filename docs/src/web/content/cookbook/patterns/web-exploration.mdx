...
```bash
bun add hono ai @ai-sdk/openai @agentuity/sandbox@beta @agentuity/keyvalue@beta @agentuity/telemetry@beta arktype zod
```
...
export async function explore(options: ExploreOptions): Promise<ExplorationResult> {
  ...
  try {
    ...
    if (!model) {
      throw new Error('Set OPENAI_MODEL to the exploration model this workflow should use.');
    }
