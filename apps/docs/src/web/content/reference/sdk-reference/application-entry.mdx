...
|-------|------|---------|-------------|
| `cors` | `CorsConfig` | Reflects any origin | Override default CORS settings (see [CORS configuration](#cors-configuration)) |
| `compression` | `CompressionConfig \| false` | Enabled, 1024 byte threshold | Configure response compression, or `false` to disable (see [Compression](#compression)) |
...
| `analytics` | `AnalyticsConfig` | Enabled | Configure analytics collection |
| `workbench` | `WorkbenchConfig` | Enabled in dev | Configure the Workbench UI |
| `router` | `Hono \| RouteMount \| RouteMount[]` | File-based routing | User-provided Hono router(s) instead of file-based routing (see [Custom Router](#custom-router)). **Experimental** |
...
To subscribe to lifecycle events, use the top-level `addEventListener` import:

```typescript
import { addEventListener } from '@agentuity/runtime';
```

See [Event System](/reference/sdk-reference/events) for available events and usage.
...
app.logger.info(`Server running at ${app.server.url}`);
...
**With Cleanup on Shutdown**

Use `registerShutdownHook` to run cleanup logic when the server stops. See [Shutdown Hooks](#shutdown-hooks) for details.
...
| *...rest* | | | Standard [Hono CORS options](https://hono.dev/docs/middleware/builtin/cors) (`origin`, `credentials`, etc.) |
...
export default await createApp({ compression: false });
```
...
The runtime exports top-level `addEventListener` and `removeEventListener` functions for subscribing to lifecycle events such as `server:started`, `agent:started`, and `agent:completed`:

```typescript
import { addEventListener, removeEventListener } from '@agentuity/runtime';
```

See the [Event System](/reference/sdk-reference/events) page for available events and usage.
...
Returns the global application state object, if one was set via an external state management mechanism.
...
Use `registerShutdownHook` to register cleanup functions that run during graceful shutdown. Hooks run in reverse order (LIFO) after agent shutdowns have completed.
...
1. Agent shutdown handlers
2. Registered shutdown hooks (reverse order, LIFO)
