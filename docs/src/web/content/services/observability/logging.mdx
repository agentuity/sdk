...
Use your framework's normal stdout, console, or structured logger for process logs. Use `@agentuity/telemetry` when framework code needs Agentuity's logger and telemetry export setup. Hono apps can use `@agentuity/hono` when route handlers should read the same logger from `c.var.logger`.
...
If your app already uses Pino, Winston, console JSON logs, a platform log drain, or a vendor agent, keep that logger. Add OpenTelemetry spans where timing and cross-service context matter.
...
During local development, `agentuity dev` runs your framework's own `dev` script with Agentuity environment behavior, so stdout and console output appear in the terminal. In deployed apps, inspect logs by session or deployment.
