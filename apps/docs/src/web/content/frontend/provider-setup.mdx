...
Once configured, `useAPI` and `useWebsocket` automatically include auth tokens in requests. See [Adding Authentication](/frontend/authentication) for full setup.
...
- [React Hooks](/frontend/react-hooks): Learn about `useAPI`, `useWebsocket`, and `useEventStream`
