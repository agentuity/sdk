...
describe('Primitive Schemas', () => {
	describe('string', () => {
		...
		test('email() should validate email format', () => {
			...
			expect(emailSchema.parse('test.user+tag@example.co.uk')).toBe(
				'test.user+tag@example.co.uk'
			);
			expect(() => emailSchema.parse('invalid')).toThrow(ValidationError);
			expect(() => emailSchema.parse('no-at-sign.com')).toThrow(ValidationError);
			expect(() => emailSchema.parse('@example.com')).toThrow(ValidationError);
			expect(() => emailSchema.parse('user@')).toThrow(ValidationError);
		});
	});
