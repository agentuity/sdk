...
The OpenAI Agents SDK enables tracing by default in server runtimes. If Agentuity should be your trace source, set `OPENAI_AGENTS_DISABLE_TRACING=1`, configure the OpenAI runner with `tracingDisabled: true`, or call `setTracingDisabled(true)` during process startup.
</Callout>
