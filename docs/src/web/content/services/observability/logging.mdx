...
Use `@agentuity/telemetry` when framework code needs structured server logs. Hono apps can use `@agentuity/hono` when route handlers should read the same logger from `c.var.logger`.
