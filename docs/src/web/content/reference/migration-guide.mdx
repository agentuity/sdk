...
```bash
npx @agentuity/migrate@alpha [project-dir] [options]
```
...
The deprecated form still works but defaults to `auth: false`. Set `auth: true` to require signed requests (recommended for deployed apps). See [Scheduling Cron Jobs](/services/schedules) for the full cron route API.
