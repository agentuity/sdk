...
import { type Env, webrtc } from '@agentuity/runtime';
...
<Callout type="info" title="React Examples Use @agentuity/react">
The React examples below use `useWebRTCCall()` from `@agentuity/react`, which is the highest-level hook for WebRTC UI state. For lower-level browser transports, media sources, or non-React usage, import `WebRTCManager` and related utilities from `@agentuity/frontend`.
</Callout>
...
```typescript
import { UserMediaSource, DisplayMediaSource, CustomStreamSource } from '@agentuity/frontend';
```
...
- [React Hooks](/frontend/react-hooks): `useWebRTCCall` and other hooks from `@agentuity/react`
