...
import {
    stream as honoStream,
    streamSSE as honoStreamSSE,
} from 'hono/streaming';
...
type AgentHandler<
    ...
    I extends Input = {}
> = (c: Context<E, P, I>) => any | Promise<any>;

type StreamHandler<
    E extends Env = Env,
    P extends string = string,
    I extends Input = {}
> = (c: Context<E, P, I>) => ReadableStream<any> | Promise<ReadableStream<any>>;
...
type WebSocketHandler<
    ...
    P extends string = string,
    I extends Input = {}
> = (c: Context<E, P, I>) => (ws: WebSocketConnection) => void | Promise<void>;

type SSEHandler<
    E extends Env = Env,
    P extends string = string,
    I extends Input = {}
> = (c: Context<E, P, I>) => (stream: any) => void | Promise<void>;
...
type ExtendedHono<E extends Env = Env, S extends Schema = {}> = {
    ...
    [K in HttpMethod]: {
        <P extends string = string>(path: P, handler: AgentHandler<E, P>): Hono<
            E,
            S
        >;
    ...
    stream<P extends string = string>(
        path: P,
        handler: StreamHandler<E, P>
    ): Hono<E, S>;
    ...
    websocket<P extends string = string>(
        path: P,
        handler: WebSocketHandler<E, P>
    ): Hono<E, S>;
    ...
    sse<P extends string = string>(
        path: P,
        handler: SSEHandler<E, P>
    ): Hono<E, S>;
...
>(): ExtendedHono<E, S> => {
    ...
    _router.websocket = (path: string, ...args: any[]) => {
        ...
        const wrapper = honoUpgradeWebSocket((c: Context) => {
            ...
            const setupFn =
                typeof setupResult === 'function' ? setupResult : null;
