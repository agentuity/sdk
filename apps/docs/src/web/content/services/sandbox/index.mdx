...
| `copilot:latest` | GitHub Copilot CLI agent |
| `cursor:latest` | Cursor AI coding agent |
| `gemini:latest` | Google Gemini CLI agent |
...
<Callout type="info" title="Server Utility">
...
When you call `sandbox.execute()` on a suspended sandbox, the sandbox automatically resumes before running the command. The response includes `autoResumed: true` so you can detect and log this behavior.
...
<Callout type="info" title="Auto-resume vs explicit resume">
