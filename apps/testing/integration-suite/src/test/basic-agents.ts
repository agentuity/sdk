...
test('basic', 'async-handler', async () => {
	...
	assert(result.elapsed >= delay, `Elapsed time (${result.elapsed}) should be >= delay (${delay})`);
	assert(actualElapsed >= delay, `Actual elapsed (${actualElapsed}) should be >= delay (${delay})`);
});
