...
For chat-style tool loops that span requests, see [State and Memory](/build/agents/state-and-memory).

```bash
npm install ai@latest @ai-sdk/anthropic@latest @agentuity/keyvalue@next zod
```
...
| Use | Good fit |
|-----|----------|
| [AI SDK](#ai-sdk) | one typed tool loop across providers, minimal boilerplate |
...
The AI SDK handles the tool loop internally. `generateText` calls your `execute` function, injects the result into the next turn, and continues until the model stops or `stopWhen` fires. All tools are validated with `inputSchema` before `execute` runs.
...
This section uses the Anthropic AI SDK provider because AI SDK owns the tool loop. Under `agentuity dev`, `ANTHROPIC_API_KEY` and `ANTHROPIC_BASE_URL` can route through AI Gateway from `AGENTUITY_SDK_KEY` when no provider key override is set. For structured JSON without an AI SDK tool loop, use [`AIGatewayClient.completeStructured()`](/services/ai-gateway#structured-output).
...
<Callout type="warning" title="Make approvals replay-safe">
...
```bash
npm install openai @agentuity/keyvalue@next zod
```
...
```bash
npm install @anthropic-ai/sdk @agentuity/keyvalue@next zod
```
...
- [State and Memory](/build/agents/state-and-memory): persist tool results and pending approvals across requests
- [Background Work](/build/apps-and-apis/background-work): enqueue tool actions that should outlive the HTTP request
- [Key-Value Storage](/services/storage/key-value): keep compact state by namespace and key
- [Queues](/services/queues): run approved side effects outside the request lifecycle
