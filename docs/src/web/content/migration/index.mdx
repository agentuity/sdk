...
<Tabs items={["v2 → v3", "v1 → v2"]} defaultValue="v2 → v3">
  ...
  <Tab value="v1 → v2">
    ...
    ```bash
    npx @agentuity/migrate@beta --v1-to-v2 --dry-run
    ```
...
| You want the exact command, flags, and generated changes | [Migration CLI](/migration/migrate-cli): inspect `npx @agentuity/migrate@beta` before applying it |

<Callout type="info" title="Using a v2 runtime app?">
Apps using `@agentuity/runtime`, `createApp()`, or `createAgent()` continue to work. See the [v2 docs](https://v2.agentuity.dev) for that model, or [Migration](/migration) to move to v3.
</Callout>
...
| you are starting a new app | create a v3 app with `bun create agentuity@beta` |
