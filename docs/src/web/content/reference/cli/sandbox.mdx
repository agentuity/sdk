...
Before you can SSH into a sandbox, you must add your SSH public key to your Agentuity account. See [SSH Access](/reference/cli/debugging#ssh-access) for setup instructions.
</Callout>
...
| `--paused-timeout <duration>` | Maximum paused time before termination (e.g., `24h`) |
| `--runtime <runtime>` | Runtime name (e.g., `bun:1`, `python:3.14`) |
| `--runtime-id <id>` | Runtime ID (e.g., `srt_xxx`) |
| `--name <name>` | Sandbox name |
| `--description <text>` | Sandbox description |
...
| `--project-id <id>` | Associate with project |
| `--org-id <id>` | Use a specific organization |
| `--wait` | Wait until the sandbox is ready before returning |
| `--wait-ms <milliseconds>` | Maximum wait time when `--wait` is used |
| `--disable-snapshots` | Skip workspace disk snapshots for faster I/O |
...
| Option | Description |
|--------|-------------|
| `--wait-for-status <status>` | Wait until the sandbox reaches a target status |
| `--wait-ms <milliseconds>` | Maximum wait time when waiting for a status |
| `--json` | JSON output |
...
agentuity cloud sandbox create --runtime bun:1 --port 3000 --name my-api
# ✓ created sandbox sbx_abc123 in 1200ms
# ℹ url: https://my-api-abc123.agentuity.cloud

# Upload server code
agentuity cloud sandbox fs cp ./server.js sbx_abc123:/home/agentuity/server.js
...
List runtimes with `agentuity cloud sandbox runtime list` before choosing `--runtime`. The default runtime may not include Bun or Node.js.
...
| Option | Description |
|--------|-------------|
| `--file <file>` | Path to build file (defaults to `agentuity-snapshot.[json|yaml|yml]`) |
| `--tag <tag>` | Snapshot tag (defaults to `latest`) |
...
agentuity cloud sandbox job create sbx_abc123 -- npm run build
