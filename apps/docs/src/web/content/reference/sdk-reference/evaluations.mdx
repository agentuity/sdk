...
Eval files are discovered at build time from `eval.ts` files next to your agents. No special imports needed in your routes.
...
// On error: throw — the runtime wraps it automatically
```
...
export const externalEval = agent.createEval('external-validation', {
  ...
  handler: async (ctx, input, output) => {
    try {
      ...
      throw new Error(`Validation service error: ${message}`);
    }
  },
});
```
