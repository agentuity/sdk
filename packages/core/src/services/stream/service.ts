 ...
 */
export const CreateStreamPropsSchema = z.object({
	/**
 ...
 */
export const ListStreamsParamsSchema = ListParamsSchema(StreamSortFieldSchema).extend({
	/**
	 * optional namespace filter to search for streams
	 */
	namespace: z.string().optional().describe('optional namespace filter to search for streams'),

	/**
	 * optional name filter to search for streams
	 */
	name: z.string().optional().describe('optional name filter to search for streams'),
	...
	metadata: z
		...
		.optional()
		.describe('optional metadata filters to match streams'),
});
...
export const StreamInfoSchema = z.object({
	 ...
	 */
	namespace: z.string().describe('the namespace of the stream'),

	/**
	 * the stream metadata
	 */
	metadata: z.record(z.string(), z.string()).describe('the stream metadata'),

	/**
	 * the public URL to access the stream
	 */
	url: z.string().describe('the public URL to access the stream'),

	/**
	 * the size of the stream in bytes
	 */
	sizeBytes: z.number().describe('the size of the stream in bytes'),
	...
	expiresAt: z
		.string()
		.nullable()
		.describe('ISO 8601 timestamp when stream expires, or null if stream never expires'),
});
...
export const ListStreamsResponseSchema = z.object({
	 ...
	 */
	message: z.string().optional().describe('optional error message if not successful'),

	/**
	 * array of streams matching the filter criteria
	 */
	streams: z.array(StreamInfoSchema).describe('array of streams matching the filter criteria'),

	/**
	 * total count of streams matching the filter (useful for pagination)
	 */
	total: z.number().describe('total count of streams matching the filter (useful for pagination)'),
});
