...
Use tasks when work needs a durable lifecycle: status, priority, assignee, comments, tags, attachments, and history. Start with `TaskClient` from any server runtime, route handler, script, or worker.
...
If your app already uses Hono, `@agentuity/hono` can initialize `TaskClient` once and expose it on `c.var.task`.
