...
`createSession()` returns session state, not completed files. Keep the `sessionId`, store the expected output paths in your own app state, then poll or subscribe until the session reaches a state your workflow considers complete. Use the retrieval path approved for your product surface; do not assume a file read helper exists unless your SDK version exposes one.
