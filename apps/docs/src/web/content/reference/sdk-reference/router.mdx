...
Routes are created using `new Hono<Env>()` from `hono` and `@agentuity/runtime`.
...
Frontend code accesses services through routes using `hc()` from `hono/client` or `fetch`:
...
See [RPC Client](/frontend/rpc-client) for type-safe `hc()` patterns and [React Hooks](/frontend/react-hooks) for auth and analytics hooks.
