...
describe('injectViteBaseFlag', () => {
	...
	test('appends to compound shell commands that invoke vite', () => {
		expect(injectViteBaseFlag('tsc -b && vite build', base)).toBe(
			`tsc -b && vite build --base=${base}`
		);
	});

	test('no-op when --base already present', () => {
...
describe('prepareViteCdnBuild', () => {
	...
	test('injects --base and CDN env, then cleanup restores command', () => {
	...
	test('patches vite.config when build command does not invoke vite', () => {
