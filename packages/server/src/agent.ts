    ...
    const handler = async (ctx: Context, input?: any) => {
        ...
        if (inputSchema) {
            ...
            if (inputResult.issues) {
                throw new Error(
                    `Input validation failed: ${inputResult.issues
                        .map((i) => i.message)
                        .join(', ')}`
                );
            }
        ...
        const agentCtx: AgentContext = new RequestAgentContext(
            (ctx as any).agent
        );
        ...
        if (outputSchema) {
            const outputResult = await outputSchema['~standard'].validate(
                result
            );
            if (outputResult.issues) {
                throw new Error(
                    `Output validation failed: ${outputResult.issues
                        .map((i) => i.message)
                        .join(', ')}`
                );
            }
