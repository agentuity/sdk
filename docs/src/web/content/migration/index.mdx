...
<Tabs items={["v2 → v3", "v1 → v2"]} defaultValue="v2 → v3">
  ...
  <Tab value="v1 → v2">
    ...
    ```bash
    npx @agentuity/migrate --v1-to-v2 --dry-run
    ```
...
| You want the exact command, flags, and generated changes | [Migration CLI](/migration/migrate-cli): inspect `npx @agentuity/migrate` before applying it |
