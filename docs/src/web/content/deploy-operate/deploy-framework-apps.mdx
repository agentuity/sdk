...
These examples show `agentuity ...` for readability. If the CLI is project-local, run the same command through your package-manager exec wrapper, for example `npx agentuity ...`, `bunx agentuity ...`, `pnpm exec agentuity ...`, or `yarn exec agentuity ...`. See [Local versus global CLI](/get-started/installation#local-versus-global-cli) for install and undo commands.
...
### Exclude monorepo paths from the deploy bundle

Monorepo deploys copy the workspace root into the staging tree so `workspace:*` dependencies resolve at runtime. That can pull in sibling apps, docs, and other non-runtime weight. Add a `.agentuityignore` file (gitignore syntax) at the monorepo root and/or inside the app package:

```gitignore title=".agentuityignore"
# Sibling apps not needed at runtime
apps/mobile/
apps/desktop/

# Docs, CI, and local tooling
docs/
.github/
.turbo/
coverage/
...
- Patterns are **relative to the monorepo root**, even when the file is `apps/web/.agentuityignore`
- Built-in exclusions always drop `node_modules`, `.git`, `.env*`, `.agentuity`, and similar (not overridable with `!`)
- Do not exclude workspace packages your app needs for `workspace:*` resolution
- Bare `dist/` would also match `apps/web/dist`; the CLI keeps the target package’s build output and warns
- Staging is wiped on every pack/deploy so new ignore rules take effect immediately

Full reference (built-ins, merge order, gotchas): [Monorepo staging and `.agentuityignore`](/reference/cli/deployment#monorepo-staging-and-agentuityignore).

### Inspect the deploy package without uploading

```bash
agentuity deploy --pack-only --log-level=trace
agentuity deploy --pack-only --pack-output ./out/deploy.zip
```

Pack-only builds the same staging tree deploy would upload, writes `agentuity-deploy.zip` (or `--pack-output`), and skips encrypt/upload. Trace logs list staging exclusions and every zip entry. Inspect with `unzip -l agentuity-deploy.zip`.

See [Inspect the package without uploading](/reference/cli/deployment#inspect-the-package-without-uploading).
...
Point the same app directory at different cloud projects with `--project-config` and layered env files via global `--env`:
...
Each config file holds that environment's `projectId`, `orgId`, `region`, domains, and resources. See [Multiple environments](/reference/cli/deployment#multiple-environments) and [Layered env files](/deploy-operate/environment-variables#layered-env-files-with-env).
...
Hono apps use the generic path, so they need explicit `build` and `start` scripts. See [Hono Apps](/frameworks/hono) for the build-script setup.
...
<Callout type="tip" title="Use package scripts for deploy workflows">
