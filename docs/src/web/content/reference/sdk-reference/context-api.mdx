---
title: Context API (v2 Compatibility)
short_title: Context API
description: Compatibility reference for the v2 runtime AgentContext ctx.* object
---

This page documents the v2 runtime `AgentContext` shape from `@agentuity/runtime` and `createAgent()` apps. In v3 framework apps, use standalone service clients, [Observability](/services/observability), and [Using Standalone Packages](/reference/standalone-packages) instead of treating `ctx.*` as the default API.

<Callout type="info" title="Keep this for existing runtime apps">
Use this page when maintaining a v2 runtime app. For new v3 code, put Agentuity clients where your framework initializes server dependencies, or use `@agentuity/hono` when Hono route handlers should read services from `c.var.*`.
</Callout>

## Context Properties

The context object passed to a v2 runtime agent handler contains the following properties:
...
Each service on the v2 runtime context object is documented on its own reference page:
...
In this v2 runtime shape, agents read services from `ctx.*` and routes read them from `c.var.*`. In v3 apps, prefer direct clients or `@agentuity/hono`; see [Using Standalone Packages](/reference/standalone-packages) for the current access patterns.
...
- `app`: Reserved app-level shared state. Defaults to `{}`, so prefer module-scoped dependencies, service objects, or `registerShutdownHook()` in runtime code.

**Agent Calling:**
- Import agents directly: `import otherAgent from '@agent/other-agent/agent'`
- Call with: `await otherAgent.run(input)`

For orchestration patterns, see [Calling Other Agents](/patterns/agents-as-a-pattern).
...
For v2 runtime apps, `ctx.config` is the stable, documented place for agent-local setup results. The runtime types still include `ctx.app`, but it defaults to `{}`. Prefer module-scoped shared dependencies plus `registerShutdownHook()` for cleanup instead of new examples built around app-level setup state.
</Callout>
