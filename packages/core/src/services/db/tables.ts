...
export const TablesResponseSchema = APIResponseSchema(
	z.object({
		tables: z.array(TableSchemaSchema).describe('Array of table schemas in the database.'),
	})
);
...
const DbTablesRequestSchema = z.object({
...
type DbTablesRequest = z.infer<typeof DbTablesRequestSchema>;
...
export async function dbTables(
	...
	const url = `/resource/${orgId}/${region}/${database}/tables`;
