...
Run custom steps before or after deployment using `predeploy` and `postdeploy` scripts in `package.json`. See [Running Deploy Lifecycle Scripts](/reference/cli/deployment#running-deploy-lifecycle-scripts).
