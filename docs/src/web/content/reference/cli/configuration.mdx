...
Layer multiple files with global `--env` (later files override earlier keys):
...
API keys authenticate requests to Agentuity APIs. Store project API keys in your `.env` as `AGENTUITY_SDK_KEY`:

```bash
AGENTUITY_SDK_KEY=agt_...
```

For the difference between project SDK keys and CLI login credentials, see [Agentuity Keys](/get-started/app-configuration#agentuity-keys).
</Callout>
