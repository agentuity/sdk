...
Attach a custom domain to a project so the deployed app serves traffic at your hostname instead of the default `*.agentco.run` URL. The domain lives in `agentuity.json`. The CLI validates DNS at deploy time, and the platform issues a TLS certificate on the first request.
...
|-------------|--------|----------|
| CNAME | `p<hash>.agentco.run` (per-project) | The provider supports CNAMEs at the level you're configuring |
