...
Add a rewrite that proxies API traffic to `agentuity dev`. This example runs Next.js on `3001` and Agentuity on `3501` to avoid port conflicts:
...
This pattern does not use `@agentuity/frameworks` or `src/generated/frameworks.ts`. The frontend imports `type ApiRouter` directly from the backend source file, and `hc()` infers requests from that type.
</Callout>
...
<Callout type="tip" title="Use hc directly for this pattern">
...
`NEXT_PUBLIC_AGENTUITY_BASE_URL` must be a full absolute URL, including the scheme, such as `https://my-app.agentuity.run`. `new URL()` throws if you pass a bare host or a relative path.
