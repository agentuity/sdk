...
<Tabs items={["v0 → v1", "v1 → v2"]} defaultValue="v1 → v2" hashSync>
<Tab value="v0 → v1">
...
5. Set S3 credentials via environment variables: see [Object Storage setup](/services/storage/object#setup)
...
## v1 Changes: Explicit Routing

Within v1, the SDK is transitioning from file-based route auto-discovery to explicit routing. This is not a breaking change, but you will see a CLI migration notice if your project has multiple route files in `src/api/`.

### What Changed

**File-based routing (previous default):** The build system scans `src/api/**/*.ts` at build time and auto-discovers route files. No configuration needed.

**Explicit routing (new):** You create a root router in `src/api/index.ts` that imports and mounts sub-routers, then pass it to `createApp({ router })`. This gives you control over route composition and mount paths.
...
This generates `src/api/index.ts` with all your existing routes wired up, and updates `src/app.ts` to pass the router to `createApp()`. Your existing route files are not modified.

See [Explicit Routing](/routes/explicit-routing) for the full guide on forms, composition, and use cases.

### Why This Change

Explicit routing aligns with standard Hono composition patterns. It makes route ordering visible in your code, supports custom mount paths and API versioning, and enables end-to-end type safety with Hono RPC.
...
2. **Review Examples**: Browse the [Cookbook](/cookbook) for working patterns
