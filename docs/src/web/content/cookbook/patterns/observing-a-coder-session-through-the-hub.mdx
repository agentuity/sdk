...
try {
  for await (const message of subscribeToCoderHub({
    ...
    if (message.type === 'broadcast') {
      ...
      logger.info('Broadcast event:', message.event);
    } else if (message.type === 'presence') {
      // Presence fires when a participant joins or leaves the session
      logger.info('Presence:', message.event);
    }
  }
} finally {
...
while (true) {
  ...
  for (const ev of events) {
    ...
    logger.info(`[${ev.occurredAt}] ${ev.category ?? '-'} / ${ev.event} (agent: ${ev.agent ?? 'none'})`);
  }
...
logger.info(`Rehydrated replay for session ${replay.sessionId}`);
```
