// @generated
// Auto-generated by Agentuity - DO NOT EDIT
import type statePersistenceAgent from '../agent/state/agent.js';
import type stateReaderAgent from '../agent/state/reader-agent.js';
import type stateWriterAgent from '../agent/state/writer-agent.js';
import type { InferInput, InferOutput } from '@agentuity/core';

// ============================================================================
// Route Schema Type Exports
// ============================================================================

/**
 * Input type for route: POST /api/agent/state
 */
export type POSTApiAgentStateInput = InferInput<typeof statePersistenceAgent['inputSchema']>;

/**
 * Output type for route: POST /api/agent/state
 */
export type POSTApiAgentStateOutput = InferOutput<typeof statePersistenceAgent['outputSchema']>;

/**
 * Input schema type for route: POST /api/agent/state
 */
export type POSTApiAgentStateInputSchema = typeof statePersistenceAgent extends { inputSchema?: infer I } ? I : never;

/**
 * Output schema type for route: POST /api/agent/state
 */
export type POSTApiAgentStateOutputSchema = typeof statePersistenceAgent extends { outputSchema?: infer O } ? O : never;

/**
 * Input type for route: POST /api/agent/state-reader
 */
export type POSTApiAgentStateReaderInput = InferInput<typeof stateReaderAgent['inputSchema']>;

/**
 * Output type for route: POST /api/agent/state-reader
 */
export type POSTApiAgentStateReaderOutput = InferOutput<typeof stateReaderAgent['outputSchema']>;

/**
 * Input schema type for route: POST /api/agent/state-reader
 */
export type POSTApiAgentStateReaderInputSchema = typeof stateReaderAgent extends { inputSchema?: infer I } ? I : never;

/**
 * Output schema type for route: POST /api/agent/state-reader
 */
export type POSTApiAgentStateReaderOutputSchema = typeof stateReaderAgent extends { outputSchema?: infer O } ? O : never;

/**
 * Input type for route: POST /api/agent/state-writer
 */
export type POSTApiAgentStateWriterInput = InferInput<typeof stateWriterAgent['inputSchema']>;

/**
 * Output type for route: POST /api/agent/state-writer
 */
export type POSTApiAgentStateWriterOutput = InferOutput<typeof stateWriterAgent['outputSchema']>;

/**
 * Input schema type for route: POST /api/agent/state-writer
 */
export type POSTApiAgentStateWriterInputSchema = typeof stateWriterAgent extends { inputSchema?: infer I } ? I : never;

/**
 * Output schema type for route: POST /api/agent/state-writer
 */
export type POSTApiAgentStateWriterOutputSchema = typeof stateWriterAgent extends { outputSchema?: infer O } ? O : never;

// ============================================================================
// Route Definitions
// ============================================================================

/**
 * Route Definitions
 * 
 * Type-safe route registry for all API routes, WebSocket connections, and SSE endpoints.
 * Used by @agentuity/react for client-side type-safe routing.
 * 
 * @remarks
 * This module augmentation is auto-generated from your route files during build.
 * Individual route Input/Output types are exported above for direct usage.
 */

declare module '@agentuity/react' {
	/**
	 * API Route Registry
	 * 
	 * Maps route keys (METHOD /path) to their input/output schemas
	 */
	export interface RouteRegistry {
	'GET /api/agent-ids/all': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'DELETE /api/agent-ids/clear': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/agent-ids/last': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'POST /api/agent-ids/run': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/agent-ids/verify/:sessionId': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: { sessionId: string };
	};
	'POST /api/agent/state': {
		inputSchema: POSTApiAgentStateInputSchema;
		outputSchema: POSTApiAgentStateOutputSchema;
		stream: typeof statePersistenceAgent extends { stream?: infer S } ? S : false;
		pathParams: never;
	};
	'POST /api/agent/state-reader': {
		inputSchema: POSTApiAgentStateReaderInputSchema;
		outputSchema: POSTApiAgentStateReaderOutputSchema;
		stream: typeof stateReaderAgent extends { stream?: infer S } ? S : false;
		pathParams: never;
	};
	'POST /api/agent/state-writer': {
		inputSchema: POSTApiAgentStateWriterInputSchema;
		outputSchema: POSTApiAgentStateWriterOutputSchema;
		stream: typeof stateWriterAgent extends { stream?: infer S } ? S : false;
		pathParams: never;
	};
	'POST /api/auth/login': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'POST /api/auth/logout': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/auth/verify': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/custom-name/custom': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'POST /api/custom-name/test': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/health': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/middleware-test/analytics-info': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/middleware-test/check-all': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/middleware-test/check-auth': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/middleware-test/query-database': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/my-service/info': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/my-service/status': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/test/list': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/test/run': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/test/suites': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'GET /api/users/profile': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'PATCH /api/users/profile': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'DELETE /api/users/profile': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	}
	
	/**
	 * WebSocket Route Registry
	 * 
	 * Maps WebSocket route paths to their schemas
	 */
	export interface WebSocketRouteRegistry {
	'/api/ws/broadcast': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'/api/ws/counter': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'/api/ws/echo': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	}
	
	/**
	 * Server-Sent Events Route Registry
	 * 
	 * Maps SSE route paths to their schemas
	 */
	export interface SSERouteRegistry {
	'/api/sse/abort-test': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'/api/sse/counter': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'/api/sse/events': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'/api/sse/long-lived': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	'/api/sse/simple': {
		inputSchema: never;
		outputSchema: never;
		stream: false;
		pathParams: never;
	};
	}

	/**
	 * RPC Route Registry
	 * 
	 * Nested structure for RPC-style client access (e.g., client.hello.post())
	 * Used by createClient() from @agentuity/core for type-safe RPC calls.
	 */
	export interface RPCRouteRegistry {
		agent: {
			state: {
				/**
				 * Route: POST /api/agent/state
				 */
				post: { input: POSTApiAgentStateInput; output: POSTApiAgentStateOutput; type: 'api'; pathParams: never };
			};
			stateReader: {
				/**
				 * Route: POST /api/agent/state-reader
				 */
				post: { input: POSTApiAgentStateReaderInput; output: POSTApiAgentStateReaderOutput; type: 'api'; pathParams: never };
			};
			stateWriter: {
				/**
				 * Route: POST /api/agent/state-writer
				 */
				post: { input: POSTApiAgentStateWriterInput; output: POSTApiAgentStateWriterOutput; type: 'api'; pathParams: never };
			};
		};
		agentIds: {
			all: {
				/**
				 * Route: GET /api/agent-ids/all
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			clear: {
				/**
				 * Route: DELETE /api/agent-ids/clear
				 */
				delete: { input: never; output: never; type: 'api'; pathParams: never };
			};
			last: {
				/**
				 * Route: GET /api/agent-ids/last
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			run: {
				/**
				 * Route: POST /api/agent-ids/run
				 */
				post: { input: never; output: never; type: 'api'; pathParams: never };
			};
			verify: {
				sessionId: {
					/**
					 * Route: GET /api/agent-ids/verify/:sessionId
					 */
					get: { input: never; output: never; type: 'api'; pathParams: { sessionId: string } };
				};
			};
		};
		auth: {
			login: {
				/**
				 * Route: POST /api/auth/login
				 */
				post: { input: never; output: never; type: 'api'; pathParams: never };
			};
			logout: {
				/**
				 * Route: POST /api/auth/logout
				 */
				post: { input: never; output: never; type: 'api'; pathParams: never };
			};
			verify: {
				/**
				 * Route: GET /api/auth/verify
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
		};
		customName: {
			custom: {
				/**
				 * Route: GET /api/custom-name/custom
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			test: {
				/**
				 * Route: POST /api/custom-name/test
				 */
				post: { input: never; output: never; type: 'api'; pathParams: never };
			};
		};
		health: {
			/**
			 * Route: GET /api/health
			 */
			get: { input: never; output: never; type: 'api'; pathParams: never };
		};
		middlewareTest: {
			analyticsInfo: {
				/**
				 * Route: GET /api/middleware-test/analytics-info
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			checkAll: {
				/**
				 * Route: GET /api/middleware-test/check-all
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			checkAuth: {
				/**
				 * Route: GET /api/middleware-test/check-auth
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			queryDatabase: {
				/**
				 * Route: GET /api/middleware-test/query-database
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
		};
		myService: {
			info: {
				/**
				 * Route: GET /api/my-service/info
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			status: {
				/**
				 * Route: GET /api/my-service/status
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
		};
		sse: {
			abortTest: {
				/**
				 * Route: GET /api/sse/abort-test
				 */
				eventstream: { input: never; output: never; type: 'sse'; pathParams: never };
			};
			counter: {
				/**
				 * Route: GET /api/sse/counter
				 */
				eventstream: { input: never; output: never; type: 'sse'; pathParams: never };
			};
			events: {
				/**
				 * Route: GET /api/sse/events
				 */
				eventstream: { input: never; output: never; type: 'sse'; pathParams: never };
			};
			longLived: {
				/**
				 * Route: GET /api/sse/long-lived
				 */
				eventstream: { input: never; output: never; type: 'sse'; pathParams: never };
			};
			simple: {
				/**
				 * Route: GET /api/sse/simple
				 */
				eventstream: { input: never; output: never; type: 'sse'; pathParams: never };
			};
		};
		test: {
			list: {
				/**
				 * Route: GET /api/test/list
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			run: {
				/**
				 * Route: GET /api/test/run
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
			suites: {
				/**
				 * Route: GET /api/test/suites
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
			};
		};
		users: {
			profile: {
				/**
				 * Route: DELETE /api/users/profile
				 */
				delete: { input: never; output: never; type: 'api'; pathParams: never };
				/**
				 * Route: GET /api/users/profile
				 */
				get: { input: never; output: never; type: 'api'; pathParams: never };
				/**
				 * Route: PATCH /api/users/profile
				 */
				patch: { input: never; output: never; type: 'api'; pathParams: never };
			};
		};
		ws: {
			broadcast: {
				/**
				 * Route: GET /api/ws/broadcast
				 */
				websocket: { input: never; output: never; type: 'websocket'; pathParams: never };
			};
			counter: {
				/**
				 * Route: GET /api/ws/counter
				 */
				websocket: { input: never; output: never; type: 'websocket'; pathParams: never };
			};
			echo: {
				/**
				 * Route: GET /api/ws/echo
				 */
				websocket: { input: never; output: never; type: 'websocket'; pathParams: never };
			};
		};
	}
}

/**
 * Runtime metadata for RPC routes.
 * Contains route type information for client routing decisions.
 * @internal
 */
const _rpcRouteMetadata = {
		"agent": {
				"state": {
						"post": {
								"type": "api",
								"path": "/api/agent/state"
						}
				},
				"stateReader": {
						"post": {
								"type": "api",
								"path": "/api/agent/state-reader"
						}
				},
				"stateWriter": {
						"post": {
								"type": "api",
								"path": "/api/agent/state-writer"
						}
				}
		},
		"agentIds": {
				"all": {
						"get": {
								"type": "api",
								"path": "/api/agent-ids/all"
						}
				},
				"clear": {
						"delete": {
								"type": "api",
								"path": "/api/agent-ids/clear"
						}
				},
				"last": {
						"get": {
								"type": "api",
								"path": "/api/agent-ids/last"
						}
				},
				"run": {
						"post": {
								"type": "api",
								"path": "/api/agent-ids/run"
						}
				},
				"verify": {
						"sessionId": {
								"get": {
										"type": "api",
										"path": "/api/agent-ids/verify/:sessionId",
										"pathParams": [
												"sessionId"
										]
								}
						}
				}
		},
		"auth": {
				"login": {
						"post": {
								"type": "api",
								"path": "/api/auth/login"
						}
				},
				"logout": {
						"post": {
								"type": "api",
								"path": "/api/auth/logout"
						}
				},
				"verify": {
						"get": {
								"type": "api",
								"path": "/api/auth/verify"
						}
				}
		},
		"customName": {
				"custom": {
						"get": {
								"type": "api",
								"path": "/api/custom-name/custom"
						}
				},
				"test": {
						"post": {
								"type": "api",
								"path": "/api/custom-name/test"
						}
				}
		},
		"health": {
				"get": {
						"type": "api",
						"path": "/api/health"
				}
		},
		"middlewareTest": {
				"analyticsInfo": {
						"get": {
								"type": "api",
								"path": "/api/middleware-test/analytics-info"
						}
				},
				"checkAll": {
						"get": {
								"type": "api",
								"path": "/api/middleware-test/check-all"
						}
				},
				"checkAuth": {
						"get": {
								"type": "api",
								"path": "/api/middleware-test/check-auth"
						}
				},
				"queryDatabase": {
						"get": {
								"type": "api",
								"path": "/api/middleware-test/query-database"
						}
				}
		},
		"myService": {
				"info": {
						"get": {
								"type": "api",
								"path": "/api/my-service/info"
						}
				},
				"status": {
						"get": {
								"type": "api",
								"path": "/api/my-service/status"
						}
				}
		},
		"sse": {
				"abortTest": {
						"eventstream": {
								"type": "sse",
								"path": "/api/sse/abort-test"
						}
				},
				"counter": {
						"eventstream": {
								"type": "sse",
								"path": "/api/sse/counter"
						}
				},
				"events": {
						"eventstream": {
								"type": "sse",
								"path": "/api/sse/events"
						}
				},
				"longLived": {
						"eventstream": {
								"type": "sse",
								"path": "/api/sse/long-lived"
						}
				},
				"simple": {
						"eventstream": {
								"type": "sse",
								"path": "/api/sse/simple"
						}
				}
		},
		"test": {
				"list": {
						"get": {
								"type": "api",
								"path": "/api/test/list"
						}
				},
				"run": {
						"get": {
								"type": "api",
								"path": "/api/test/run"
						}
				},
				"suites": {
						"get": {
								"type": "api",
								"path": "/api/test/suites"
						}
				}
		},
		"users": {
				"profile": {
						"delete": {
								"type": "api",
								"path": "/api/users/profile"
						},
						"get": {
								"type": "api",
								"path": "/api/users/profile"
						},
						"patch": {
								"type": "api",
								"path": "/api/users/profile"
						}
				}
		},
		"ws": {
				"broadcast": {
						"websocket": {
								"type": "websocket",
								"path": "/api/ws/broadcast"
						}
				},
				"counter": {
						"websocket": {
								"type": "websocket",
								"path": "/api/ws/counter"
						}
				},
				"echo": {
						"websocket": {
								"type": "websocket",
								"path": "/api/ws/echo"
						}
				}
		}
} as const;

// Store metadata globally for createAPIClient() to access
if (typeof globalThis !== 'undefined') {
	(globalThis as Record<string, unknown>).__rpcRouteMetadata = _rpcRouteMetadata;
}

/**
 * Type-safe API client is available from @agentuity/react
 *
 * @example
 * ```typescript
 * import { createAPIClient } from '@agentuity/react';
 *
 * const api = createAPIClient();
 * const result = await api.hello.post({ name: 'World' });
 * ```
 */

// FOUND AN ERROR IN THIS FILE?
// Please file an issue at https://github.com/agentuity/sdk/issues
// or if you know the fix please submit a PR!
