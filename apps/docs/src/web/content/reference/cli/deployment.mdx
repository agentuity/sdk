...
<Callout type="warning" title="Only Applies via bun run deploy">
`predeploy` and `postdeploy` are standard npm lifecycle hooks, not Agentuity-specific. They run when you invoke `bun run deploy` (or `npm run deploy`). They do **not** run when calling `agentuity deploy` directly.
</Callout>
