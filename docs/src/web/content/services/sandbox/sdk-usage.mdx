...
try {
  const execution = await sandbox.execute({
    command: ['bun', '-e', 'console.log("line 1"); console.error("err 1")'],
    pipe: {
      stdout: process.stdout,
      stderr: process.stderr,
    },
...
try {
  ...
  if (pauseResult.terminatesAt !== undefined) {
    console.log('paused sandbox terminates at', pauseResult.terminatesAt);
  }
} finally {
  await sandbox.destroy();
}
...
Pass `'0'` for `timeout.paused` to allow the sandbox to remain paused indefinitely. The CLI flag is `--paused-timeout` on `agentuity cloud sandbox create`.
...
for (const entry of lineage.lineage) {
  console.log(entry.snapshotId, entry.name, entry.createdAt);
}
```
...
| `timeout.paused` | `string` | Max duration a sandbox can remain paused before termination, such as `'24h'`; `'0'` for infinite |
