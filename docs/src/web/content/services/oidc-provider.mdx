...
`refreshToken()` does not require `clientSecret` — confidential client secrets are included if present in config, but the call succeeds without them (the server decides whether to require the secret).
...
Per RFC 7009, the revocation endpoint returns success even if the token was already invalid. Prefer revoking the refresh token over the access token — revoking the refresh token invalidates the entire grant.
