...
<Callout type="info" title="Validate against the managed service">
In-memory or mock implementations are useful for constructor and API-shape tests. Validate managed-service behavior, search relevance, TTLs, and permissions against the managed service before relying on those details.
</Callout>
