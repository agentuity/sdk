...
<Callout type="tip" title="Add AgentuityProvider only when you need hooks">
Plain `hc()` calls do not require `AgentuityProvider`. Add the provider when you want auth, analytics, or WebRTC hooks from `@agentuity/react`.
</Callout>
...
```tsx
const client = hc<ApiRouter>(process.env.NEXT_PUBLIC_AGENTUITY_BASE_URL!);
```
