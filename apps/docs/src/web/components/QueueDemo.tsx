...
export function QueueDemo() {
	...
	const handleCheckDlq = async () => {
		...
		try {
			const result = await api<{
				success: boolean;
				data?: { messages: DlqMessage[]; total?: number };
			}>('/dlq');
			if (result.success && result.data) {
				setDlqMessages(result.data.messages);
				addEvent(
					'dlq',
					`DLQ: ${result.data.messages.length} message(s)${result.data.total !== undefined ? ` (total: ${result.data.total})` : ''}`
				);
			}
		} catch (err) {
			setError(err instanceof Error ? err.message : 'Failed to check DLQ');
		} finally {
			setLoading(null);
		}
	};
