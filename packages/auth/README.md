...
- **Agentuity (BetterAuth)** - `@agentuity/auth/agentuity` - First-class auth with database-backed sessions
- **Clerk** - `@agentuity/auth/clerk` - Third-party auth provider
- WorkOS - Coming soon
- Auth0 - Coming soon
...
The recommended auth provider for Agentuity projects. Uses BetterAuth with Agentuity PostgreSQL for database-backed sessions, organizations, API keys, and JWT tokens.
...
4. **Keep secrets secret** - Never commit `.env` files or expose `CLERK_SECRET_KEY` or `BETTER_AUTH_SECRET`
5. **Use environment variables** - Store all keys in environment variables, not code
6. **Rotate secrets periodically** - Rotate `BETTER_AUTH_SECRET` on a regular schedule
7. **Control OTEL PII** - Use `otelSpans: { email: false }` to exclude sensitive data from telemetry when not needed
