...
Switch profiles with `--profile <name>` or `AGENTUITY_PROFILE=...`. Your framework's runtime reads its own variables. Without `--env`, the CLI parses these files mainly for the SDK key it needs to wire the gateway during `agentuity dev`.

### Layered env files with `--env`

Use the global `--env` flag to load one or more env files explicitly. Files are applied in the order you pass them; **later files override earlier keys**. Values are merged into `process.env` for the CLI process.

```bash
# Shared base, then environment-specific overrides
agentuity --env .env --env .env.staging deploy
agentuity --env .env --env .env.staging cloud env push
agentuity --env .env.production cloud env list
```

Comma-separated paths work too: `--env .env,.env.staging`.

When `--env` is set, deploy and `cloud env push` sync from those files (merged) instead of only `.env`. Pair this with `--project-config` when each environment is a different Agentuity project:
...
See [Multiple environments](/reference/cli/deployment#multiple-environments) for the full multi-project pattern.

<Callout type="info" title="The CLI does not load every local var for your framework">
`agentuity dev` patches `AGENTUITY_SDK_KEY` and three provider env pairs (see below). Your app reads everything else through your framework's own loader, your shell, or a process manager. Keep `.env` for shared values and `.env.local` for personal overrides. When you pass `--env`, the CLI process also receives those file values.
</Callout>

## AI Gateway in Local Dev

Agentuity's AI Gateway provider catalog can change by project and release. Use [AI Gateway](/services/ai-gateway#provider-ids) or `agentuity cloud aigateway models --simple` for the current provider IDs. Local `agentuity dev` has one automatic env-injection path today.
...
See [AI Gateway local development](/services/ai-gateway#local-development) for env injection and [Bring your own provider key](/services/ai-gateway#bring-your-own-provider-key) for BYOK options.
...
# Push a layered local env set (later files win)
agentuity --env .env --env .env.staging cloud env push
```
...
`push` reads `.env` by default (or the files from global `--env` when provided), drops reserved Agentuity keys (see below), classifies the rest, and writes to the cloud project. Without `--force`, the CLI prompts before overwriting an existing cloud value.
