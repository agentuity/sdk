...
export async function generateRouteRegistry(
	...
	// Ensure the file is a module (not a script) so that `declare module` blocks
	// are module augmentations rather than ambient module declarations.
	// Without any import/export, TypeScript treats the file as a script and
	// `declare module` replaces the target module's types instead of extending them.
	const hasImports = importsStr.trim() || typeImports.trim() || shouldEmitFrontendClient;
	const moduleMarker = hasImports ? '' : '\nexport {};\n';
