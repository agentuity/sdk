...
`;
	...
	// .env loading code - MUST happen before imports for SDKs that check env at import time
	// Using synchronous file operations with absolute paths derived from __dirname
	const envLoading = `
// CRITICAL: Load .env files BEFORE any imports (some SDKs check env vars at import time)
// Using synchronous operations since this must complete before module imports
import { readFileSync, existsSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';

const __dirname = dirname(fileURLToPath(import.meta.url));
const envFiles = [join(__dirname, '.env'), join(__dirname, '.env.local')];
