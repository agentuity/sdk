...
```bash
npm install hono @agentuity/keyvalue@next zod @agentuity/telemetry@next
```
...
- [Agents](/build/agents): read memory before model calls
- [Chat and Streaming](/build/agents/chat-and-streaming): persist chat history after streamed responses
- [Key-Value Storage](/services/storage/key-value): configure TTL, inspect namespaces, and understand sliding expiration
- [Migrating from Runtime Apps](/migration/runtime-to-frameworks): map `ctx.*` state access from previous SDK versions to explicit app state
