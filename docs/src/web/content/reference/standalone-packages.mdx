...
Install the v3 package tag shown in these examples until the npm `latest` tag points at the framework-first SDK. Omitting the tag can install older v2 packages.
</Callout>
...
| Package | Install | Use |
|---------|---------|-----|
| [`@agentuity/keyvalue`](#key-value-storage) | `bun add @agentuity/keyvalue@beta` | exact-key data, TTL-backed cache, preferences, counters |
| [`@agentuity/vector`](#vector-search) | `bun add @agentuity/vector@beta` | documents, embeddings, semantic search, retrieval |
| [`@agentuity/storage`](#object-storage) | `bun add @agentuity/storage@beta` | S3-compatible file storage under Bun or Node.js |
| [`@agentuity/stream`](#durable-streams) | `bun add @agentuity/stream@beta` | durable, resumable streams and generated files |
| [`@agentuity/aigateway`](#ai-gateway) | `bun add @agentuity/aigateway@beta` | model catalog lookup and routed model requests |
| [`@agentuity/queue`](#message-queues) | `bun add @agentuity/queue@beta` | async handoff to worker or pub/sub queues |
| [`@agentuity/email`](#email) | `bun add @agentuity/email@beta` | managed email addresses, outbound email, inbound records |
| [`@agentuity/schedule`](#schedules) | `bun add @agentuity/schedule@beta` | cron schedules and delivery tracking |
| [`@agentuity/task`](#tasks) | `bun add @agentuity/task@beta` | work items, comments, tags, attachments, activity |
| [`@agentuity/sandbox`](#sandbox) | `bun add @agentuity/sandbox@beta` | isolated command execution and long-lived sandboxes |
| [`@agentuity/webhook`](#webhooks) | `bun add @agentuity/webhook@beta` | managed webhook ingest URLs, destinations, receipts |
| [`@agentuity/coder`](#coder) | `bun add @agentuity/coder@beta` | Coder sessions, workspaces, custom agents, replay data |
| [`@agentuity/db`](#database) | `bun add @agentuity/db@beta` | trusted admin scripts against the database service API |
| [`@agentuity/hono`](#hono) | `bun add @agentuity/hono@beta hono` | Hono middleware that injects common clients on `c.var.*` |
| [`@agentuity/telemetry`](#telemetry) | `bun add @agentuity/telemetry@beta` | logger, tracer, and meter setup outside Hono |
...
```bash
bun add @agentuity/hono@beta hono
```
...
```bash
bun add @agentuity/storage@beta
```
...
if (!model) {
  throw new Error('AGENTUITY_AI_MODEL is required');
}
...
```bash
bun add @agentuity/sandbox@beta
```
...
```bash
bun add @agentuity/webhook@beta
```
...
```bash
bun add @agentuity/schedule@beta
```
...
```bash
bun add @agentuity/telemetry@beta
```
