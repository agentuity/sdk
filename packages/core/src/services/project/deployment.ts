...
export const DeploymentMetadataSchema = z.object({
	git: z
		.object({
			...
			trigger: z.string().optional().describe('What triggered the deployment (e.g., push, pr, manual).'),
			...
			event: z.string().optional().describe('Git event that triggered the build (e.g., push, pull_request).'),
			pull_request: z
				.object({
...
export const DeploymentSchema = z.object({
	...
	state: z.string().optional().describe('Current state of the deployment (e.g., building, deployed, failed).'),
	...
	updatedAt: z.string().nullable().optional().describe('ISO 8601 timestamp when the deployment was last updated.'),
	metadata: DeploymentMetadataSchema.optional().describe('Git and build metadata for the deployment.'),
	customDomains: z.array(z.string()).nullable().optional().describe('Custom domains configured for this deployment.'),
	cloudRegion: z.string().nullable().optional().describe('Cloud region where the deployment is running.'),
	resourceDb: z.string().nullable().optional().describe('Database resource identifier for the deployment.'),
	resourceStorage: z.string().nullable().optional().describe('Storage resource identifier for the deployment.'),
	deploymentLogsURL: z.string().nullable().optional().describe('URL to view deployment runtime logs.'),
...
export const DeploymentActionResponseSchema = APIResponseSchemaOptionalData(
	z.object({ activeDeploymentId: z.string().optional().describe('ID of the currently active deployment after the action.') })
);
