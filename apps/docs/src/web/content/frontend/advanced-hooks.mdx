...
For most use cases, `hc()` from `hono/client` or standard `fetch` work directly with your routes. These advanced patterns are for WebRTC and custom real-time integrations.
</Callout>
...
|----------|------------------|
| HTTP API routes | `hc()` from `hono/client` or `fetch` |
