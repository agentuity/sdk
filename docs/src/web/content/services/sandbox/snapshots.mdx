...
Manage snapshots via the [Web App](https://app.agentuity.com) under **Services > Sandbox > Snapshots**, the [SDK](/services/sandbox/sdk-usage), or [CLI](/reference/cli/sandbox).
...
<Callout type="info" title="Snapshots are different from checkpoints">
Snapshots are reusable bases for new sandboxes. Checkpoints belong to one sandbox and support pause, resume, restore, and delete workflows.
</Callout>
...
The generated file includes documentation for runtime selection, apt dependencies, npm/bun packages, file patterns, environment variables, and metadata. Customize it for your needs, then build:
...
packages:  # npm/bun packages to install globally
  - typescript
...
```bash
# Build with a custom build file
...
agentuity cloud sandbox snapshot create sbx_abc123 --name python-ml --description "Python data tools"
...
For manual snapshots created from a sandbox, set runtime environment variables when you create or run the next sandbox. Declarative snapshot builds can include `env` values in the build file, or substitute them at build time with `--env KEY=VALUE`.
