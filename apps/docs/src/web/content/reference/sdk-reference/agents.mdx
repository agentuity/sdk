...
Agent `setup()` and `shutdown()` are current and useful. The type system still includes app-level state plumbing, but the current `createApp()` guidance is better centered on module-scoped shared dependencies plus `registerShutdownHook()` than on new examples built around `ctx.app`.
</Callout>
...
For streaming patterns with routes, see [HTTP Routes](/routes/http).
