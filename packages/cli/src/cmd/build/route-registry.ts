...
`;
	...
	if (generatedDir) {
		...
		// Generate for src/_generated/ (nested structure: method.path.segments)
		const { imports: srcImports, agentImports: srcAgentImports } = generateImports(false);

		// Build nested tree structure for API routes
		// Structure: { post: { api: { hello: { inputSchema, outputSchema, stream } } } }
		type RouteTree = { [key: string]: RouteTree | RouteInfo };
		const apiTree: RouteTree = {};
		...
		for (const route of apiRoutes) {
			...
			// Initialize method level if needed
			if (!apiTree[method]) {
				apiTree[method] = {};
			}

			// Navigate/create the nested structure
			let current = apiTree[method] as RouteTree;
			for (let i = 0; i < pathSegments.length; i++) {
				const segment = pathSegments[i];
				const isLast = i === pathSegments.length - 1;

				if (isLast) {
					// Store the route info at the leaf
		...
		// Helper to generate the leaf route entry (schema types)
		const generateLeafEntry = (route: RouteInfo, indent: string): string => {
			if (!route.hasValidator) {
				const streamValue = route.stream === true ? 'true' : 'false';
				return `{
${indent}  inputSchema: never;
${indent}  outputSchema: never;
${indent}  stream: ${streamValue};
${indent}}`;
			}

			if (route.agentVariable) {
				const importName = srcAgentImports.get(route.agentVariable)!;
				return `{
${indent}  inputSchema: typeof ${importName} extends { inputSchema?: infer I } ? I : never;
${indent}  outputSchema: typeof ${importName} extends { outputSchema?: infer O } ? O : never;
${indent}  stream: typeof ${importName} extends { stream?: infer S } ? S : false;
${indent}}`;
			}
			...
			if (route.inputSchemaVariable || route.outputSchemaVariable) {
				...
				const streamValue = route.stream === true ? 'true' : 'false';
				return `{
${indent}  inputSchema: ${inputType};
${indent}  outputSchema: ${outputType};
${indent}  stream: ${streamValue};
${indent}}`;
			}

			return `{
${indent}  inputSchema: any;
${indent}  outputSchema: any;
${indent}  stream: false;
${indent}}`;
		};

		// Helper to check if a value is a RouteInfo (leaf) or nested tree
		const isRouteInfo = (val: RouteTree | RouteInfo): val is RouteInfo => {
			return 'method' in val && 'path' in val && 'hasValidator' in val;
		};

		// Recursive function to serialize the tree to TypeScript
		const serializeTree = (tree: RouteTree, indent: string): string => {
			const entries: string[] = [];

			for (const [key, value] of Object.entries(tree)) {
				if (isRouteInfo(value)) {
					// Leaf node - generate the schema entry
					entries.push(`${indent}${key}: ${generateLeafEntry(value, indent)};`);
				} else {
					// Branch node - recurse
					entries.push(`${indent}${key}: {
${serializeTree(value, indent + '  ')}
${indent}};`);
				}
			}
		...
		const apiRouteEntries = serializeTree(apiTree, '  ');

		// For WebSocket and SSE, keep simpler flat structure for now
		const websocketEntries = websocketRoutes
			.map((route) => {
				const key = route.path.replace(/^\//, '').replace(/\//g, '_');
				return `  ${key}: ${generateLeafEntry(route, '  ')};`;
			})
			.join('\n');

		const sseEntries = sseRoutes
			.map((route) => {
				const key = route.path.replace(/^\//, '').replace(/\//g, '_');
				return `  ${key}: ${generateLeafEntry(route, '  ')};`;
			})
