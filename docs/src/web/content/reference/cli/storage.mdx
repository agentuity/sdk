...
Use these commands to inspect and manage Agentuity storage from the terminal. For app code, start with [Storage](/services/storage). For framework routes, workers, scripts, or CLIs, use [Using Standalone Packages](/reference/standalone-packages).
...
For app code, see [Key-Value Storage](/services/storage/key-value). For direct-client setup, see [Using Standalone Packages](/reference/standalone-packages).
...
For app code, use Bun's `s3` API (`import { s3 } from "bun"`). See [Object Storage (S3)](/services/storage/object).
...
For app code, see [Vector Storage](/services/storage/vector). For direct-client setup, see [Using Standalone Packages](/reference/standalone-packages).
...
For app code, use Bun's `sql` API (`import { sql } from "bun"`). See [Database](/services/database).
...
For app code, see [Durable Streams](/services/storage/durable-streams). For direct-client setup, see [Using Standalone Packages](/reference/standalone-packages).
...
- [Storage](/services/storage): Choose a storage type and v3 access pattern
- [Using Standalone Packages](/reference/standalone-packages): Use direct service clients from framework routes, workers, scripts, or CLIs
- [Key-Value Storage](/services/storage/key-value): Store exact-key cache and state
- [Object Storage (S3)](/services/storage/object): Store files and binary blobs with Bun S3
- [Database](/services/database): Manage relational Postgres resources and app-owned SQL data
