...
├── agentuity.config.ts   # Minimal config (may be empty — CLI needs this file)
...
- **`agentuity.config.ts`** — Minimal config file. **Required** — the CLI needs this file to recognize the project. Runtime settings (workbench, analytics) go in `createApp()` in `app.ts`. Build settings go in `vite.config.ts`:
  ```typescript
  export default {};
  ```
...
10. **Create `agentuity.config.ts`** in the project root (can be empty — the CLI requires this file to recognize the project):
    ```typescript
    export default {};
    ```
    If you had Vite plugins in `agentuity.config.ts`, move them to `vite.config.ts`.
...
- **AI Gateway** — LLM requests (OpenAI, Anthropic, etc.) automatically route through Agentuity's AI Gateway using the SDK key. **No separate API keys needed.** Just use the AI SDK or the provider SDK (e.g. `new OpenAI()`) normally — it works out of the box.
