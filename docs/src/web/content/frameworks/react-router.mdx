...
Use the model `id` exactly as it appears in the [AI Gateway model catalog](/services/ai-gateway#model-catalog). Use a provider SDK directly only when the resource route depends on provider-specific APIs.
...
| Model call returns an auth error | Verify `AGENTUITY_SDK_KEY` is available to the resource route. See [AI Gateway](/services/ai-gateway) |
