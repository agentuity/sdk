...
Use `@agentuity/react` when you want React-aware access to Agentuity frontend state: provider context, transport auth state, analytics, and WebRTC. For route calls, use `hc()` from `hono/client` or plain `fetch`.
...
- [Provider Setup](/frontend/provider-setup): Configure `AgentuityProvider` and transport auth state
- [Advanced Hooks](/frontend/advanced-hooks): Use low-level managers for custom WebSocket and SSE clients
