...
Each standalone invocation creates a fresh thread and session. Persist IDs in your own data layer when you need to reconnect later work to a previous run.
...
2. **Create thread and restore session**: A fresh thread is created for the invocation, and the session is restored or created using the session ID.
