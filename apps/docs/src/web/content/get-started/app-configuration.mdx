...
Files loaded later in the chain override earlier ones. During local development the loading order is `.env.development` then `.env`, so `.env` is loaded last and its values take precedence when both files define the same key.
