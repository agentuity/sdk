...
interface DocumentMetadata {
  ...
  readonly category: 'guide' | 'api' | 'tutorial';
...
router.post('/ask',
  ...
  async (c) => {
    ...
    const results = await c.var.vector.search<KnowledgeMetadata>('knowledge-base', {
      query: question,
      limit: 3,
      similarity: 0.7,
    });
...
async function indexDocuments(files: readonly string[]): Promise<void> {
