...
<Callout type="info" title="Works Inside or Outside HTTP">
`createAgentContext()` works in standalone scripts, workers, and external backends. If you call it before `createApp()`, Agentuity starts a minimal standalone runtime automatically. Call `createApp()` first when you want your full app config, providers, or cloud-connected services loaded.
</Callout>
...
| `thread` | `Thread` | Existing thread to reuse |
| `session` | `Session` | Existing session to reuse |

By default, each standalone invocation creates a fresh thread and session. Pass `thread` or `session` when you need to reuse conversation state that you already manage, or persist IDs in your own data layer to reconnect later work.
...
2. **Create or reuse conversation state**: By default, a fresh thread is created and the session is restored or created using the session ID. If you pass `thread` or `session`, those objects are reused.
...
5. **Wait for background tasks**: Any `ctx.waitUntil()` callbacks are awaited before the invocation completes.
6. **Save session and thread**: Session and thread state are persisted via their providers.
7. **Fire session complete event**: A `complete` event is sent with the final status code, agent IDs that participated, and any user data.
