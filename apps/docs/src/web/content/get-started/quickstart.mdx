...
Wire that route and your agent into the project-root `app.ts`:
...
Workbench is a built-in UI for testing agents directly, without going through your frontend. Enable it in `createApp()`:
...
Import your router type into the frontend and use `hc<ApiRouter>()` when you want end-to-end typed API calls without extra client code generation.
