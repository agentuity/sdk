...
- use key-value storage or a database for v3 app state, not v2 runtime thread state

<Callout type="info" title="Existing v2 thread-state apps can keep running">
...
- [Chat and Streaming](/patterns/chat-and-streaming): stream assistant text while storing the final transcript
- [Key-Value Storage](/services/storage/key-value): configure TTLs, search keys, and manage namespaces
- [Migrating Runtime Apps to Frameworks](/migration/runtime-to-frameworks): move v2 agent runtime state into v3 app-owned state
