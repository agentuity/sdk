...
| `workbench` | `boolean \| string \| WorkbenchOptions` | Deprecated development UI option kept for older apps |
...
`analytics` accepts `true`, `false`, or an options object. `workbench` remains available for older apps and still accepts `true`, a custom route string like `'/debug'`, or a full config object.
