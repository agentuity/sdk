...
async function main() {
	...
	try {
		...
		createdWebhookIds.push(webhook3.id);
		...
		// Best-effort cleanup: delete every webhook we created. The
		// service drops associated destinations along with the webhook.
		if (createdWebhookIds.length > 0) {
			...
			for (const id of createdWebhookIds) {
				try {
					...
					console.log(`   ⚠️  Could not delete webhook ${id}: ${msg}`);
				}
			}
		}
	}
