...
<Callout type="tip" title="Keep scheduled handlers short">
Point the schedule at a route that validates the request and publishes durable work to a queue when the job may run longer than a normal HTTP request. The schedule delivery stays easy to retry, and the queue owns the slower work.
</Callout>
...
- [Schedules API Reference](/reference/api/schedules): inspect REST fields and delivery response shapes
