---
title: Using Sandboxes (v2 Compatibility)
short_title: Sandbox Service
description: Compatibility reference for v2 runtime ctx.sandbox APIs
---

This page documents v2 runtime sandbox access through `ctx.sandbox` in agents and `c.var.sandbox` in runtime routes. In v3 apps, start with [Running Code in Sandboxes](/services/sandbox), [Sandbox SDK Usage](/services/sandbox/sdk-usage), and [Using Standalone Packages](/reference/standalone-packages#sandbox): use `SandboxClient` from `@agentuity/sandbox`.

<Callout type="info" title="Use SandboxClient for new v3 code">
The `ctx.sandbox` examples below still work for existing v2 runtime apps. New v3 code usually creates a `SandboxClient` from `@agentuity/sandbox`.
</Callout>
...
| `projectId` | `string` | no | Project ID to associate the sandbox with |
...
| `stream` | `SandboxStreamConfig` | no | Output stream IDs for stdout, stderr, and stdin |
...
| `scopes` | `string[]` | no | Permission scopes for automatic service access |
...
interface Sandbox {
  ...
  auditStreamId?: string;       // Audit event stream ID, when enabled
