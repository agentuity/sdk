...
const endpoints = [
	{
		...
		explanation: (
			<>
				Access per-request state via <code>ctx.session.state</code> and the
				unique session ID via <code>ctx.session.id</code>. State resets after
				each request.
			</>
		),
	...
	},
	{
		...
		explanation: (
			<>
				Check which services are available: <code>ctx.kv</code>,{" "}
				<code>ctx.vector</code>, <code>ctx.logger</code>,{" "}
				<code>ctx.tracer</code>. All are initialized automatically.
			</>
		),
	...
	},
	{
		...
		explanation: (
			<>
				List all registered agents and invoke them with{" "}
				<code>agent.run(input)</code>. Both routes and agents can call <i>other</i> agents for complex
				workflows.
			</>
		),
		codeHint: "ctx.agents",
	},
	{
		...
		explanation: (
			<>
				Thread state persists across requests (via cookies). Use{" "}
				<code>ctx.thread.state</code> to store conversation history or user
				preferences.
			</>
		),
	...
	},
	{
		...
		explanation: (
			<>
				Direct access to the full context object showing session, thread, and
	...
	},
	{
		...
		explanation: (
			<>
				Structured logging with <code>ctx.logger.info()</code>,{" "}
				<code>.warn()</code>, <code>.error()</code>, <code>.trace()</code>. Logs
				appear in the Agentuity console.
			</>
		),
	...
	},
	{
		...
		explanation: (
			<>
				Run tasks after the response is sent with{" "}
				<code>ctx.waitUntil(promise)</code>. Perfect for analytics, cleanup, or
				slow operations.
			</>
		),
