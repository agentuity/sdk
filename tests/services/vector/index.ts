...
async function main() {
	...
	try {
		...
		// Best-effort cleanup: drop the entire namespace, which removes
		// the four upserted documents in one call.
		if (needsCleanup) {
			console.log('\n🗑️  Cleaning up...');
			try {
				...
				console.log(`   ⚠️  Could not delete namespace ${NAMESPACE}: ${msg}`);
			}
		}
	}
}
