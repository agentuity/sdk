...
```bash
bun add ai@latest @ai-sdk/openai@latest @agentuity/keyvalue@beta zod
```
...
if (!model) {
  throw new Error('Set OPENAI_MODEL to the model you want this workflow to use.');
}
...
Keep the model ID in configuration so you can change extraction, classification, review, and reasoning workflows independently. Use [AI Gateway](/services/ai-gateway#model-catalog) or your provider's catalog to pick the exact model ID.
</Callout>
...
Use this table only when you are moving an older runtime app into v3. New code should start with the framework column.

| Concern | v3 framework app | Older runtime app |
|---|---|---|
...
See [Migrating Runtime Apps to Frameworks](/migration/runtime-to-frameworks) when a current app still has runtime-managed agents.
...
```bash
bun add hono @agentuity/hono@beta zod
```
