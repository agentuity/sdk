...
Use this page when you maintain code that already uses `@agentuity/schema`. New v3 examples should generally use Zod, ArkType, or Valibot because Agentuity accepts Standard Schema-compatible validators.

<Callout type="warning" title="@agentuity/schema is deprecated">
`@agentuity/schema` remains available for existing projects, but new v3 docs and starters should prefer Zod, ArkType, or Valibot. Keep using this reference when you are migrating older code or need the package-specific helpers below.
</Callout>
...
try {
  ...
  if (err instanceof ValidationError) {
    ...
    logger.info('Validation issues', err.issues);
    // [
...
if (result.success) {
  logger.info('Parsed user', result.data); // { name: 'Alice', age: 30 }
} else {
  logger.info('Validation issues', result.error.issues); // ValidationError with issues array
}
```
