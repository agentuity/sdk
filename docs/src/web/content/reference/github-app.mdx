...
When the Agentuity GitHub App builds your project, it downloads your source code, installs dependencies, and runs deploy lifecycle scripts around `agentuity deploy` from an isolated build environment.

### Default Behavior

The platform installs dependencies before running deployment scripts:

1. Download and extract your source code
2. Run `bun install` to install dependencies
3. Run `bun run --bun predeploy` if `package.json` defines `predeploy`
4. Run `agentuity deploy` to build and deploy your project
5. Run `bun run --bun postdeploy` if `package.json` defines `postdeploy`

No extra setup is required beyond linking your repository.

### Pre-Deploy Scripts

If your `package.json` defines a `predeploy` script, it runs after dependency installation and before `agentuity deploy`:

```json title="package.json"
{
  "scripts": {
    "predeploy": "npm run build:packages",
    "deploy": "agentuity deploy"
  }
}
```

Use `predeploy` for monorepo package builds, generated assets, or validation that must happen before deployment.

### Post-Deploy Scripts

If your `package.json` defines a `postdeploy` script, it runs after `agentuity deploy` completes successfully. Use it for notifications, changelog updates, or downstream tasks.
...
These are independent from `deployment.resources`, which controls CPU, memory, and disk for your running agents. See [Runtime and build resources](/reference/cli/deployment#runtime-and-build-resources) for details.
