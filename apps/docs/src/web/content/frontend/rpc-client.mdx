...
Call your API routes with full type safety using Hono's built-in RPC client.
...
// POST with JSON body — fully typed
...
The client infers input and output types from the router, so `$post({ json: ... })` is fully typed without a separate registry.
...
|----------|-------------|
| React components | [React Hooks](/frontend/react-hooks) with `hc()` or `fetch` |
