...
| **Web App** | Visual task board, manual triage, dashboard views |
...
| `done` | Work completed | (no date field) |
| `closed` | Resolved and closed | `closed_date` |
...
`ctx.task.close(id)` is a convenience method that sets status to `'closed'` in one call. For `'done'` or `'cancelled'`, use `ctx.task.update(id, { status: '...' })` directly.
</Callout>
...
const agent = createAgent('AttachmentUploader', {
  handler: async (ctx, input) => {
    ...
    await fetch(presigned_url, { // [!code highlight]
      method: 'PUT',
...
Each `TaskChangelogEntry` records the `field` name, `old_value`, `new_value`, and `created_at` timestamp. Status transitions, priority changes, and reassignments all appear automatically — no extra instrumentation needed.
...
Set `parent_id` to build epic-to-feature-to-task hierarchies. Query subtasks with `list({ parent_id: epicId })`.
