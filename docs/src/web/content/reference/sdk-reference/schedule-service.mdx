---
title: Schedules (v2 Compatibility)
short_title: Schedule Service
description: Compatibility reference for v2 runtime ctx.schedule APIs
---

This page documents v2 runtime schedule access through `ctx.schedule` in agents and `c.var.schedule` in runtime routes. In v3 apps, start with [Schedules](/services/schedules) and [Using Standalone Packages](/reference/standalone-packages#schedules): use `ScheduleClient` from `@agentuity/schedule`.

<Callout type="info" title="Use ScheduleClient for new v3 code">
The `ctx.schedule` examples below still work for existing v2 runtime apps. New v3 code usually creates a `ScheduleClient` from `@agentuity/schedule`.
</Callout>
