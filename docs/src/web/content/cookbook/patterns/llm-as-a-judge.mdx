...
```bash
bun add ai @ai-sdk/openai @ai-sdk/anthropic @ai-sdk/groq @agentuity/queue@next @agentuity/vector@next @agentuity/telemetry@next zod
```
...
const answerModel = process.env.OPENAI_ANSWER_MODEL;
const judgeModel = process.env.GROQ_JUDGE_MODEL;

if (!answerModel || !judgeModel) {
  throw new Error('Set OPENAI_ANSWER_MODEL and GROQ_JUDGE_MODEL.');
}
...
const openaiModel = process.env.OPENAI_ARENA_MODEL;
const anthropicModel = process.env.ANTHROPIC_ARENA_MODEL;
const judgeModel = process.env.GROQ_JUDGE_MODEL;

if (!openaiModel || !anthropicModel || !judgeModel) {
  throw new Error('Set OPENAI_ARENA_MODEL, ANTHROPIC_ARENA_MODEL, and GROQ_JUDGE_MODEL.');
}
...
export async function runArena(
  ...
  const [openaiResult, anthropicResult] = await Promise.all([
    generateWithTiming(openai(openaiModel), prompt, tone),
    generateWithTiming(anthropic(anthropicModel), prompt, tone),
  ]);
...
The worker route is fired by an HTTP destination on the `answer-quality` queue. See [Background Work](/build/apps-and-apis/background-work) for the full request/status/worker shape.
...
- [Background Work](/build/apps-and-apis/background-work): wire the judge worker route to a queue with retries and status
