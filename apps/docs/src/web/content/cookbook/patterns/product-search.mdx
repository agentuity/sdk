...
const router = new Hono<Env>()
  .get('/search', async (c) => {
    ...
    const maxPriceFilter = maxPrice === undefined ? undefined : Number(maxPrice);
...
export function App() {
  ...
  const search = async () => {
    ...
    const response = await fetch('/api/products/advisor', { // [!code highlight]
      method: 'POST',
...
curl -X POST "http://localhost:3500/api/products/advisor" \
  ...
  -d '{"query":"comfortable office chair"}'
```
