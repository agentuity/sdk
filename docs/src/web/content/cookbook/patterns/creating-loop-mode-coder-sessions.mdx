...
async function main(): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (TERMINAL.has(status)) {
      ...
      if (loop.summary) {
        logger.info('Summary:', loop.summary);
      }
      return;
    }
    ...
    if (status === 'awaiting_input') {
      ...
      logger.info('Loop is awaiting input. Exiting poll loop.');
      return;
    }
  }

  logger.error('Timed out waiting for loop to complete.');
  process.exitCode = 1;
}
...
`getLoopState()` exposes `loop.summary` and `loop.nextAction` as the loop progresses, so your UI or downstream step can poll for them without holding a live controller socket.
