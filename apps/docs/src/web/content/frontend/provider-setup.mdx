...
Wrap your app in `AgentuityProvider` to share the backend base URL and optional auth header state with Agentuity's React helpers.
...
For apps with user authentication, keep `AgentuityProvider` and `AuthProvider` separate. Wrapping both is fine, but `AgentuityProvider` does not automatically consume tokens from `AuthProvider`.
