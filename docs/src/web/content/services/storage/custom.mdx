...
Most v3 apps should use the dedicated service clients directly: `KeyValueClient`, `VectorClient`, `StreamClient`, and Bun's `s3` APIs. When a feature needs another backend, hide that decision behind a small app-owned interface.
...
Object storage is not part of the key-value, vector, or stream interface set. In Bun code, use `S3Client` when you need another bucket or S3-compatible provider.
...
For Bun-only local storage checks, `@agentuity/local` exports local implementations backed by `~/.config/agentuity/local.db`.

```typescript
import { LocalKeyValueStorage, getLocalDB } from '@agentuity/local';
...
<Callout type="info" title="Local implementations are Bun-only checks">
