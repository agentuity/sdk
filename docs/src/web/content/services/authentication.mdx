...
That same boundary fits Next.js, TanStack Start, Hono, Remix, SvelteKit, Nuxt, Astro, and custom backends because Agentuity services do not require a specific auth package in the request path.
...
Existing v2 apps that use `@agentuity/auth` can keep using the [v2 docs](https://v2.agentuity.dev). For new v3 work, start with framework-owned auth or Agentuity OIDC instead of adding `@agentuity/auth` as the default session layer.
</Callout>
