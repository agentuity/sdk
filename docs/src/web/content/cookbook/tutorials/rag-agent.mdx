...
```bash
npm install hono @agentuity/aigateway@next @agentuity/vector@next valibot zod
```

`@agentuity/vector` and `@agentuity/aigateway` read Agentuity project credentials from the environment, so run the route with `agentuity dev` or a linked Agentuity project.
...
- **Stream the answer.** Use `AIGatewayClient.streamRequest()` and return a `Response` whose body is the stream. See [Chat and Streaming](/build/agents/chat-and-streaming).
